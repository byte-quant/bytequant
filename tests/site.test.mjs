...
test("exports the complete four-language site", async () => {
  ...
  for (const page of [home, english, german, chinese]) {
    ...
    assert.match(page, /ByteQuant AI 7\.7/);
  }
  ...
  assert.equal((llms.match(/^- \[/gm) ?? []).length, 342);
...
test("exports all tool and guide routes", async () => {
  ...
  assert.equal(turkishTools.filter((name) => !name.includes(".")).length, 354);
  assert.equal(englishTools.filter((name) => !name.includes(".")).length, 354);
  assert.equal(germanTools.filter((name) => !name.includes(".")).length, 354);
  assert.equal(chineseTools.filter((name) => !name.includes(".")).length, 354);
...
test("exports the four-language editorial discovery and structured-data package", async () => {
  ...
  assert.match(blog, /<strong>116<\/strong>\s*(?:<!-- -->)?\s*ayrıntılı rehber/);
  assert.match(englishBlog, /<strong>116<\/strong>\s*(?:<!-- -->)?\s*in-depth guides/);
...
test("exports the four-language local agent, domain integrity, and security headers", async () => {
  ...
  assert.match(conversationSource, /resolveAgentRequest/);
