...
test("AI 7.6 presents conversation before optional model controls without removing capabilities", async () => {
  ...
  assert.match(styles, /Product usability 7\.6/);
  assert.match(styles, /\.agent-chat-v76>\.agent-chat-stream\{order:2/);
  assert.match(styles, /\.agent-chat-v76>\.agent-model-drawer\{order:5/);
  assert.match(styles, /\.agent-model-drawer/);
  assert.match(styles, /\.agent-composer\{position:sticky/);
});

test("Community 6.1 keeps the Nostr feature set in a responsive social timeline", async () => {
  ...
  assert.match(network, /community-social-v61/);
  ...
  assert.match(styles, /\.community-social-v61 \.community-global-post>p\{font-size:15px/);
