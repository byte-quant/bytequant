...
- P2P collaboration uses WebRTC DataChannel with no ByteQuant signaling service, STUN, or TURN. Single-use SDP codes expire after ten minutes and can expose ICE candidate network information. Live document sharing is paused until users compare the same DTLS-derived safety code through a separate trusted channel and explicitly enable sharing. The safety code helps detect an active intermediary but is not identity verification.
