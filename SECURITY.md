...
- P2P collaboration and session chat use WebRTC DataChannel with no ByteQuant signaling service, STUN, TURN, account, presence, or message archive. Single-use SDP codes expire after ten minutes and can expose ICE candidate network information. Live document sharing and chat are paused until users compare the same DTLS-derived safety code through a separate trusted channel and explicitly enable them. The safety code helps detect an active intermediary but is not identity verification; chat history disappears when the session closes.
- Peer messages, concurrent transfers, pending invitations, buffered output, signal size, and document size are bounded. Malformed, expired, oversized, or room-mismatched data is rejected locally.
...
- The update feed is generated before deployment from fixed NASA/NIST HTTPS RSS endpoints. Redirect destinations and item URLs are rechecked against source host allowlists; responses are time- and size-bounded, and no feed HTML, article body, or image is rendered as trusted markup.
