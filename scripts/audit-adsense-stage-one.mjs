...
assert.equal(publicTools.length, 342, "canonical public tool count changed");
assert.equal(new Set(publicTools.map((tool) => tool.slug)).size, 342, "canonical slugs must be unique");

for (const locale of locales) {
  assert.equal(new Set(publicTools.map((tool) => tool.title[locale].trim().toLocaleLowerCase(locale))).size, 342, `${locale} titles must be unique`);
  assert.equal(new Set(publicTools.map((tool) => tool.short[locale].trim().toLocaleLowerCase(locale))).size, 342, `${locale} summaries must be unique`);
  assert.equal(new Set(publicTools.map((tool) => tool.description[locale].trim().toLocaleLowerCase(locale))).size, 342, `${locale} descriptions must be unique`);
}
...
assert.equal(indexedPages, 1368, "all 342 canonical tools must remain indexable in four languages");
...
- All 342 canonical tools remain publicly indexable in four languages; Stage 1 introduces no blanket noindex or tool removal.
