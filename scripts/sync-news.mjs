...
function itemSource(xml, feed) { const blocks = [...xml.matchAll(/<(?:item|entry)(?:\s[^>]*)?>([\s\S]*?)<\/(?:item|entry)>/gi)].map((match) => match[1]); return blocks.slice(0, 20).flatMap((block) => { const title = field(block, ["title"]).slice(0, 240); const rawUrl = link(block); const rawDate = field(block, ["pubDate", "published", "updated", "dc:date"]); if (title.length < 8 || !rawUrl || suspicious(title) || (feed.titleFilter && !feed.titleFilter.test(title))) return []; try { const url = new URL(rawUrl); if (url.protocol !== "https:" || !feed.hosts.includes(url.hostname) || url.username || url.password) return []; const date = new Date(rawDate); const timestamp = date.valueOf(); if (Number.isNaN(timestamp) || timestamp > now + maxFutureSkew || timestamp < now - maxAge) return []; const candidateSummary = shortSummary(field(block, ["description", "summary", "content:encoded", "content"])); const sourceSummary = suspicious(candidateSummary) ? "" : candidateSummary; const summaryOrigin = sourceSummary ? "feed" : "metadata"; const completeSummary = sourceSummary || shortSummary(`${feed.source} published this official ${feed.category} update under the headline “${title}”. Open the source for full context.`); return [{ id: `${feed.source.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${id(url.toString())}`, title, url: url.toString(), date: date.toISOString().slice(0, 10), source: feed.source, category: feed.category, region: feed.region, sourceLanguage: feed.sourceLanguage, sourceSummary: completeSummary, summaryOrigin }]; } catch { return []; } }); }
