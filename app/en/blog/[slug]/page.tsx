...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const post = getPost(slug); if (!post) return {}; return { title: post.title.en, description: post.description.en, keywords: ["privacy", "GDPR", "KVKK", "prompt engineering", "client-side tools"], alternates: { canonical: postPath("en", slug), languages: { "tr-TR": postPath("tr", slug), "en-US": postPath("en", slug) } }, openGraph: { type: "article", locale: "en_US", url: `${siteUrl}${postPath("en", slug)}`, title: post.title.en, description: post.excerpt.en, publishedTime: post.date, modifiedTime: post.date, authors: ["ByteQuant Editorial"] } }; }
