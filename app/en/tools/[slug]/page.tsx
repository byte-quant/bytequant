...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: tool.title.en, description: tool.description.en, robots: isToolAlias(slug) ? nonIndexableRobots : undefined, alternates: localizedAlternates("en", toolPath("tr", canonicalSlug), toolPath("en", canonicalSlug)), openGraph: { type: "website", siteName: "ByteQuant", locale: "en_US", alternateLocale: ["tr_TR", "de_DE", "zh_CN"], url: absoluteUrl(toolPath("en", canonicalSlug)), title, description: tool.description.en, images: [{ url: ogImageUrl, width: 1200, height: 630, alt: title }] }, twitter: { card: "summary_large_image", title, description: tool.description.en, images: [ogImageUrl] } };
}
