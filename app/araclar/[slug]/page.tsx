...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: tool.title.tr, description: tool.short.tr, alternates: localizedAlternates("tr", toolPath("tr", slug), toolPath("en", slug)), openGraph: { type: "website", siteName: "ByteQuant", locale: "tr_TR", alternateLocale: ["en_US", "de_DE", "zh_CN"], url: absoluteUrl(toolPath("tr", slug)), title, description: tool.short.tr, images: [{ url: ogImageUrl, width: 1200, height: 630, alt: title }] }, twitter: { card: "summary_large_image", title, description: tool.short.tr, images: [ogImageUrl] } };
}
