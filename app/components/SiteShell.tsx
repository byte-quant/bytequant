    ...
    <div className="site-shell">
      ...
      <footer className="site-footer">
        <div className="container footer-grid">
          ...
          <div><h2>{localized("Keşfet", "Explore", "Entdecken", "探索")}</h2><Link href={pathFor(locale, "tools")}>{t.nav.tools}</Link><Link href={pathFor(locale, "agent")}>{localized("Yerel Ajan", "Local Agent", "Lokaler Agent", "本地助手")}</Link><Link href={pathFor(locale, "workstation")}>{localized("İş İstasyonu", "Workstation", "Workstation", "工作站")}</Link><Link href={pathFor(locale, "blog")}>{t.nav.blog}</Link><Link href={referencePath(locale, "regex-cheat-sheet")}>Regex cheat sheet</Link><Link href={referencePath(locale, "cron-cheat-sheet")}>Cron cheat sheet</Link><Link href={pathFor(locale, "faq")}>{t.nav.faq}</Link></div>
