...
export default function WorkstationClient({ locale }: { locale: Locale }) {
  ...
  const filteredTools = useMemo(() => {
    ...
    return tools.filter((tool) => (category === "all" || tool.category === category) && (!term || `${tool.title[locale]} ${tool.slug} ${tool.short[locale]}`.toLocaleLowerCase().includes(term)));
  ...
  return <div className="workstation-app" aria-label={t.title}>
    ...
    <div className="workstation-grid"><aside className="workspace-sidebar">
      ...
      <section className="workspace-canvas-shell" aria-labelledby="canvas-title"><header><div><h3 id="canvas-title">{t.canvas}</h3><small>{connectFrom ? t.connect : `${summary.nodes}/${WORKSPACE_MAX_NODES} ${t.nodes} · ${summary.edges} ${t.flow}`}</small></div></header><div className="workspace-canvas-scroll"><div className="workspace-canvas" style={{ width: canvasSize.width, height: canvasSize.height }} onDragOver={(event) => event.preventDefault()} onDrop={dropTool} onClick={() => setConnectFrom(null)}>
