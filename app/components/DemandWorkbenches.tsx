...
const configs: Record<string, Config> = {
  ...
  "satir-siralayici-tekillestirici": { inputLabel: c("Her öğe ayrı satırda", "One item per line", "Ein Eintrag pro Zeile", "每行一个条目"), options: [
    { value: "stable-unique", label: c("Sırayı koruyarak tekilleştir", "Deduplicate and keep order", "Reihenfolge beibehalten und deduplizieren", "保持顺序去重") },
...
export function DemandWorkbench({ slug, locale }: { slug: string; locale: Locale }) {
  ...
  return <Frame locale={locale} onDemo={loadDemo} onClear={clear} busy={busy}>
    <div className="workbench-grid">
      <div className="workbench-inputs" data-agent-contract="single-text-v1">
        <label className="field-label"><span>{config.inputLabel[locale]}</span><textarea data-agent-input data-agent-key="input" value={input} onChange={(event) => { setInput(event.target.value); resetResult(); }} rows={11} maxLength={500000} spellCheck="false" /><small className="field-counter">{input.length.toLocaleString(localeTag(locale))} / 500.000</small></label>
