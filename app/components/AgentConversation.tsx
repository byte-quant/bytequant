...
export function AgentConversation({ locale }: { locale: Locale }) {
  ...
  return <section className="agent-chat-app" aria-label={t.hello}>
    ...
    {!turns.length && <div className="agent-fast-capabilities" aria-label={locale === "tr" ? "İndirmeden hazır yetenekler" : locale === "de" ? "Ohne Download verfügbare Funktionen" : locale === "zh" ? "无需下载即可使用的功能" : "Capabilities ready without a download"}><strong>{locale === "tr" ? "İndirmeden hazır" : locale === "de" ? "Ohne Download bereit" : locale === "zh" ? "无需下载" : "Ready instantly"}</strong><span>{locale === "tr" ? "Sohbet ve bağlam" : locale === "de" ? "Gespräch und Kontext" : locale === "zh" ? "对话与语境" : "Chat and context"}</span><span>{locale === "tr" ? "Hesap ve birim" : locale === "de" ? "Rechnen und Einheiten" : locale === "zh" ? "计算与换算" : "Math and units"}</span><span>{locale === "tr" ? "Özet ve açıklama" : locale === "de" ? "Zusammenfassen und erklären" : locale === "zh" ? "总结与解释" : "Summarise and explain"}</span><span>{locale === "tr" ? "327 araçta doğru rota" : locale === "de" ? "Route durch 327 Werkzeuge" : locale === "zh" ? "匹配 327 个工具" : "Route across 327 tools"}</span></div>}
