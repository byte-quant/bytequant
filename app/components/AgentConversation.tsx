...
import { AGENT_SESSION_KEY, AGENT_VERSION, createAgentPlan, readAgentSession, semanticToolSearch, translateAgentError, type AgentPlan } from "../lib/agent-core";
...
const historyKey = "bytequant:local-agent-conversation:v3";
...
const ui = {
  ...
  zh: {
    ...
    plan: "建议流程", why: "推荐理由", start: "开始第一步", workstation: "发送到可视化流程", details: "计划详情与限制", confidence: "匹配度", experts: "3 项本地检查", expertText: "工具顺序、隐私与交付质量由独立规则检查。", dataRun: "在此运行文本数据", dataIntro: "此计划的全部步骤都可自动处理文本。请先使用合成数据，而非真实个人数据。", dataPlaceholder: "待处理文本、CSV 或 JSON…", run: "在此设备运行计划", running: "正在运行…", output: "自动运行结果", copy: "复制输出", copied: "已复制", unavailable: "此流程需要选择文件或进行视觉检查，无法安全自动运行。", voiceUnavailable: "此浏览器未确认设备端语音识别；您仍可使用文本输入框。", utilities: "查找工具或解释错误", find: "查找工具", findPlaceholder: "例如：检查安全响应头", error: "解释错误", errorPlaceholder: "粘贴前请移除个人数据和机密", clear: "清除对话", previous: "已使用之前的对话作为语境。", network: "0 次远程模型调用",
  },
} as const;
...
export function AgentConversation({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
      try {
        ...
        if (session) setPlan(session.plan.locale === locale ? session.plan : createAgentPlan(session.plan.goal, tools, locale));
  ...
  function submit() {
    ...
    requestAnimationFrame(() => {
      ...
      setPlan(next); setTurns(nextTurns); setGoal(""); setBusy(false);
      try {
        sessionStorage.setItem(AGENT_SESSION_KEY, JSON.stringify({ plan: next, currentStep: 0, stepOutputs: {}, completedStepIds: [] }));
  ...
  function startGuided(index = 0) {
    if (!plan) return;
    try { sessionStorage.setItem(AGENT_SESSION_KEY, JSON.stringify({ plan, currentStep: index, stepOutputs: {}, completedStepIds: [] })); } catch { /* tool still opens */ }
  }

  function runHere() {
    if (!plan) return;
    setAutomationError(""); setAutomation(null);
  ...
  function reset() {
    setGoal(""); setPlan(null); setTurns([]); setData(""); setAutomation(null); setAutomationError("");
    try { sessionStorage.removeItem(historyKey); sessionStorage.removeItem(AGENT_SESSION_KEY); } catch { /* optional */ }
