...
const ui = {
  tr: {
    hello: "Merhaba, ben ByteQuant AI.", helloBody: "Sorunuzu veya yapmak istediğiniz işi günlük dille yazın. Sohbet eder, hesabı çözer, uygun aracı bulur; veri verdiğinizde desteklenen adımları sizin onayınızla hazırlarım.", placeholder: "Örn. Bu CSV'deki e-postaları temizle, tekrarları kaldır ve JSON olarak hazırla", send: "Gönder", thinking: "Uygun yolu hazırlıyorum…", private: "Bu konuşma bu sekmede kalır", memory: "Bağlam açık", newChat: "Yeni sohbet", voice: "Sesle yaz", listening: "Dinliyorum…", examples: [["Hızlı bir soru", "125 * 1.18 kaçtır? İşlemi de göster"], ["Veri temizle", "CSV dosyamdaki kişisel verileri maskele ve paylaşılabilir JSON hazırla"], ["Görseli PDF yap", "Fotoğraflarımı tek PDF dosyasında birleştirmek istiyorum"], ["Hata çöz", "JSON.parse Unexpected token hatasını açıkla ve doğru aracı öner"]],
  ...
  en: {
    hello: "Hi, I’m ByteQuant AI.", helloBody: "Write a question or task in everyday language. I can chat, calculate, find the right tool, and—with your approval—prepare supported steps when you provide data.", placeholder: "Example: Clean this email list, remove duplicates, and prepare JSON", send: "Send", thinking: "Building a practical path…", private: "This conversation stays in this tab", memory: "Context on", newChat: "New chat", voice: "Voice input", listening: "Listening…", examples: [["Ask a quick question", "What is 125 * 1.18? Show the calculation"], ["Clean data", "Mask personal data in my CSV and prepare shareable JSON"], ["Make a PDF", "Combine my photos into a single PDF file"], ["Fix an error", "Explain a JSON.parse Unexpected token error and recommend the right tool"]],
  ...
  de: {
    hello: "Hallo, ich bin ByteQuant AI.", helloBody: "Schreiben Sie eine Frage oder Aufgabe in Alltagssprache. Ich kann chatten, rechnen, das passende Werkzeug finden und unterstützte Schritte mit Ihrer Freigabe vorbereiten.", placeholder: "Beispiel: E-Mail-Liste bereinigen, Duplikate entfernen und JSON erstellen", send: "Senden", thinking: "Praktischen Weg erstellen…", private: "Dieses Gespräch bleibt im Tab", memory: "Kontext aktiv", newChat: "Neues Gespräch", voice: "Spracheingabe", listening: "Hört zu…", examples: [["Kurze Frage", "Wie viel ist 125 * 1,18? Rechenweg anzeigen"], ["Daten bereinigen", "Personendaten in CSV maskieren und teilbares JSON erstellen"], ["PDF erstellen", "Meine Fotos zu einer PDF-Datei zusammenführen"], ["Fehler lösen", "JSON.parse Unexpected token erklären und passendes Werkzeug empfehlen"]],
  ...
  zh: {
    hello: "您好，我是 ByteQuant AI。", helloBody: "请用日常语言写下问题或任务。我可以对话、计算、找到合适工具，并在您确认后为提供的数据准备受支持的步骤。", placeholder: "例如：清理邮件列表、去重并生成 JSON", send: "发送", thinking: "正在生成可执行路径…", private: "对话仅保留在当前标签页", memory: "语境已开启", newChat: "新对话", voice: "语音输入", listening: "正在聆听…", examples: [["快速提问", "计算 125 * 1.18，并显示算式"], ["清理数据", "遮蔽 CSV 中的个人数据并生成可分享 JSON"], ["生成 PDF", "把我的照片合并成一个 PDF 文件"], ["解决错误", "解释 JSON.parse Unexpected token 错误并推荐合适工具"]],
...
const messageCopy = {
  tr: { fastNotice: "Anında yanıt; hesaplama, özet, metin ölçümü, kavram açıklaması ve araç eşleme cihazınızda açıklanabilir kurallarla yapıldı.", upgrade: "Yerel AI ile derinleştir", copy: "Kopyala", copied: "Kopyalandı", speak: "Sesli oku", retry: "Yeniden üret", inactive: "İndirme gerektirmeyen hızlı AI hazır", active: "Gerçek yerel model etkin", toolReady: "327 araç için eşleme ve otomasyon", context: "Sekme içi konuşma bağlamı", once: "İlk kullanımda bir kez indirilir" },
...
export function AgentConversation({ locale }: { locale: Locale }) {
  ...
  return <section className={`agent-chat-app agent-chat-v74 ${turns.length ? "has-turns" : "is-empty"}`} aria-label={t.hello}>
    ...
    {!turns.length && <section className="agent-conversation-welcome"><span aria-hidden="true">✦</span><div><strong>{locale === "tr" ? "Ne yapmak istiyorsunuz?" : locale === "de" ? "Was möchten Sie erreichen?" : locale === "zh" ? "您想完成什么？" : "What would you like to accomplish?"}</strong><p>{locale === "tr" ? "Sorunuzu doğal biçimde yazın. ByteQuant AI doğrudan cevaplar; işlem gerekiyorsa doğru aracı bulur, girdiyi hazırlar ve onayınızdan sonra çalıştırır." : locale === "de" ? "Schreiben Sie natürlich. ByteQuant AI antwortet direkt und findet bei einer Aufgabe das passende Werkzeug, bereitet die Eingabe vor und wartet vor der Ausführung auf Ihre Bestätigung." : locale === "zh" ? "请用自然语言描述。ByteQuant AI 会直接回答；需要处理时，它会匹配工具、准备输入，并在执行前等待您的确认。" : "Write naturally. ByteQuant AI answers directly; when a task needs processing, it finds the right tool, prepares the input, and waits for your approval before running it."}</p></div><div className="agent-fast-capabilities" aria-label={locale === "tr" ? "İndirmeden hazır yetenekler" : locale === "de" ? "Ohne Download verfügbare Funktionen" : locale === "zh" ? "无需下载即可使用的功能" : "Capabilities ready without a download"}><span>{locale === "tr" ? "Sor ve sohbet et" : locale === "de" ? "Fragen und sprechen" : locale === "zh" ? "问答与对话" : "Ask and chat"}</span><span>{locale === "tr" ? "Yaz ve karşılaştır" : locale === "de" ? "Schreiben und vergleichen" : locale === "zh" ? "写作与比较" : "Write and compare"}</span><span>{locale === "tr" ? "Dosya veya görsel ekle" : locale === "de" ? "Datei oder Bild anhängen" : locale === "zh" ? "添加文件或图片" : "Attach a file or image"}</span><span>{locale === "tr" ? "327 aracı çalıştır" : locale === "de" ? "327 Werkzeuge nutzen" : locale === "zh" ? "使用 327 个工具" : "Use 327 tools"}</span></div></section>}

    <details className={`agent-model-drawer ${aiStatus}`}>
      <summary><span aria-hidden="true">✦</span><div><strong>{aiStatus === "ready" ? message.active : ai.automatic}</strong><small>{aiStatus === "ready" ? `${activeModelLabel} · ${ai.enabled}` : locale === "tr" ? "İsteğe bağlı cihaz içi model ve performans ayarları" : locale === "de" ? "Optionales Gerätemodell und Leistungseinstellungen" : locale === "zh" ? "可选设备端模型与性能设置" : "Optional on-device model and performance settings"}</small></div><b>{aiStatus === "ready" ? "✓" : "+"}</b></summary>
