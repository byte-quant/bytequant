...
export function AgenticAssistant({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
      ...
      try {
        ...
        if (Array.isArray(parsed)) setTurns(parsed.filter((item): item is AgentTurn => Boolean(item) && typeof item === "object" && typeof (item as AgentTurn).goal === "string" && typeof (item as AgentTurn).response === "string" && Array.isArray((item as AgentTurn).tools) && Number.isFinite((item as AgentTurn).createdAt)).slice(-12));
  ...
  const makePlan = () => {
    ...
    window.setTimeout(() => {
      ...
      const nextTurns = [...turns, { goal: nextPlan.goal, response: nextPlan.response, tools: nextPlan.steps.map((step) => step.title), createdAt: Date.now() }].slice(-12);
