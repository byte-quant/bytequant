...
function DiffWorkbench({ locale }: { locale: Locale }) {
  ...
  return (
    <WorkbenchFrame locale={locale} onDemo={() => { setOldText(isTr ? "ByteQuant veriyi yerel olarak işler.\nAraçlar ücretsizdir." : "ByteQuant processes data locally.\nThe tools are free."); setNewText(isTr ? "ByteQuant hassas veriyi tarayıcıda işler.\nAraçlar ücretsiz ve üyeliksizdir." : "ByteQuant processes sensitive data in the browser.\nThe tools are free and require no account."); setMode("words"); }} onClear={() => { setOldText(""); setNewText(""); }}>
      <div className="special-toolbar"><label><span>{isTr ? "Karşılaştırma düzeyi" : "Comparison level"}</span><select value={mode} onChange={(event) => setMode(event.target.value as "words" | "lines")}><option value="words">{isTr ? "Kelime" : "Words"}</option><option value="lines">{isTr ? "Satır" : "Lines"}</option></select></label><div className="diff-toolbar-actions"><button type="button" onClick={() => { setOldText(newText); setNewText(oldText); setNotice(null); }}>↔ {isTr ? "Metinleri değiştir" : "Swap texts"}</button><div className="diff-legend"><span className="legend-added">+ {isTr ? "Eklenen" : "Added"}: {added}</span><span className="legend-deleted">− {isTr ? "Silinen" : "Removed"}: {deleted}</span></div></div></div>
      <div className="dual-input-grid"><label className="field-label"><span>{isTr ? "Eski metin" : "Old text"}</span><textarea rows={10} maxLength={50000} value={oldText} onChange={(event) => { setOldText(event.target.value); setNotice(null); }} /><small className="field-counter">{oldText.length.toLocaleString(isTr ? "tr-TR" : "en-US")} / 50.000</small></label><label className="field-label"><span>{isTr ? "Yeni metin" : "New text"}</span><textarea rows={10} maxLength={50000} value={newText} onChange={(event) => { setNewText(event.target.value); setNotice(null); }} /><small className="field-counter">{newText.length.toLocaleString(isTr ? "tr-TR" : "en-US")} / 50.000</small></label></div>
      <div className="rich-result-panel"><div className="result-header"><span>{isTr ? "Renkli fark" : "Colored diff"}</span><div className="output-actions"><button type="button" onClick={copyDiff} disabled={!diff.parts.length || Boolean(diff.error)}>{isTr ? "Kopyala" : "Copy"}</button><button type="button" onClick={() => downloadText(plainDiff, "bytequant-diff.txt")} disabled={!diff.parts.length || Boolean(diff.error)}>{isTr ? "İndir" : "Download"}</button></div></div>{diff.error ? <ToolNotice locale={locale} notice={{ kind: "error", text: isTr ? "Karşılaştırma sınırı aşıldı. Daha küçük bir bölüm seçin (en fazla 1.400 parça)." : "The comparison limit was exceeded. Use a smaller section (up to 1,400 segments)." }} /> : <pre data-agent-output data-ready={diff.parts.length ? "true" : "false"} className={`diff-output diff-${mode}`}>{diff.parts.length ? diff.parts.map((part, index) => <span className={`diff-${part.type}`} key={`${part.type}-${index}`}>{part.value}</span>) : (isTr ? "Fark burada görünecek." : "The diff will appear here.")}</pre>}<ToolNotice notice={notice} locale={locale} /></div>
    </WorkbenchFrame>
  );
}
...
function parseColor(value: string): Rgb | null {
  ...
  if (rgb) {
    ...
    return values.every((channel) => channel >= 0 && channel <= 255) ? { r: values[0], g: values[1], b: values[2] } : null;
  }
  const hsl = input.match(/^hsla?\(\s*(-?\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)%\s*,\s*(\d+(?:\.\d+)?)%(?:\s*,[^)]+)?\)$/i);
  return hsl && Number(hsl[2]) <= 100 && Number(hsl[3]) <= 100 ? hslToRgb(Number(hsl[1]), Number(hsl[2]), Number(hsl[3])) : null;
}
