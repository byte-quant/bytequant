...
function WorkbenchFrame({ locale, children, onClear }: { locale: Locale; children: ReactNode; onClear?: () => void }) {
...
function DiffWorkbench({ locale }: { locale: Locale }) {
  ...
  return (
    <WorkbenchFrame locale={locale} onClear={() => { setOldText(""); setNewText(""); }}>
      ...
      <div className="rich-result-panel"><div className="result-header"><span>{isTr ? "Renkli fark" : "Colored diff"}</span></div>{diff.error ? <p className="tool-message error">{isTr ? "Karşılaştırma sınırı aşıldı. Daha küçük bir bölüm seçin (en fazla 1.400 parça)." : "The comparison limit was exceeded. Use a smaller section (up to 1,400 segments)."}</p> : <pre className={`diff-output diff-${mode}`}>{diff.parts.length ? diff.parts.map((part, index) => <span className={`diff-${part.type}`} key={`${part.type}-${index}`}>{part.value}</span>) : (isTr ? "Fark burada görünecek." : "The diff will appear here.")}</pre>}</div>
    </WorkbenchFrame>
  );
}
...
function MarkdownWorkbench({ locale }: { locale: Locale }) {
  ...
  async function copyHtml() { await navigator.clipboard.writeText(html); setMessage(isTr ? "HTML panoya kopyalandı." : "HTML copied to the clipboard."); }
  return (
    <WorkbenchFrame locale={locale} onClear={() => { setMarkdown(""); setMessage(""); }}>
      <div className="markdown-grid"><label className="field-label"><span>Markdown</span><textarea rows={22} maxLength={50000} value={markdown} onChange={(event) => setMarkdown(event.target.value)} spellCheck="false" /></label><div className="markdown-results"><div className="markdown-preview"><div className="result-header"><span>{isTr ? "Canlı önizleme" : "Live preview"}</span></div><article dangerouslySetInnerHTML={{ __html: html }} /></div><div className="html-code-panel"><div className="result-header"><span>HTML</span><div><button type="button" onClick={copyHtml}>{isTr ? "Kopyala" : "Copy"}</button><button type="button" onClick={() => downloadText(html, "bytequant-markdown.html", "text/html;charset=utf-8")}>{isTr ? "İndir" : "Download"}</button></div></div><pre>{html || (isTr ? "HTML burada görünecek." : "HTML will appear here.")}</pre>{message && <p className="tool-message">{message}</p>}</div></div></div>
    </WorkbenchFrame>
  );
}
...
function ColorWorkbench({ locale }: { locale: Locale }) {
  ...
  function update(input: string) { setValue(input); const parsed = parseColor(input); if (parsed) { setRgb(parsed); setMessage(""); } else setMessage(isTr ? "HEX, rgb(r,g,b) veya hsl(h,s%,l%) biçimi kullanın." : "Use HEX, rgb(r,g,b), or hsl(h,s%,l%) format."); }
  async function copy(text: string) { await navigator.clipboard.writeText(text); setMessage(isTr ? `${text} kopyalandı.` : `${text} copied.`); }
  return (
    <WorkbenchFrame locale={locale} onClear={() => update("#000000")}>
      <div className="color-workbench"><div className="color-controls"><label className="field-label color-picker-field"><span>{isTr ? "Renk seçici" : "Color picker"}</span><input type="color" value={hex} onChange={(event) => update(event.target.value)} /></label><label className="field-label"><span>{isTr ? "Renk değeri" : "Color value"}</span><input value={value} onChange={(event) => update(event.target.value)} placeholder="#4F46E5 · rgb(79,70,229) · hsl(243,75%,59%)" /></label>{message && <p className={message.includes("kopyalandı") || message.includes("copied") ? "tool-message" : "tool-message error"}>{message}</p>}</div><div className="color-swatch" style={{ backgroundColor: hex }}><span style={{ color: hsl.l > 60 ? "#111827" : "#ffffff" }}>{hex}</span></div><div className="color-values">{[["HEX", hex], ["RGB", rgbText], ["HSL", hslText]].map(([label, text]) => <button type="button" key={label} onClick={() => copy(text)}><small>{label}</small><code>{text}</code><span>{isTr ? "Kopyala" : "Copy"}</span></button>)}</div></div>
    </WorkbenchFrame>
  );
}
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (active) { setPng(nextPng); setSvg(nextSvg); setError(""); }
      } catch (reason) { if (active) { setError(reason instanceof Error ? reason.message : String(reason)); setPng(""); setSvg(""); } }
    }, 180);
    return () => { active = false; window.clearTimeout(timer); };
  }, [input]);
  function download(url: string, filename: string) { const anchor = document.createElement("a"); anchor.href = url; anchor.download = filename; anchor.click(); }
  return (
    <WorkbenchFrame locale={locale} onClear={() => setInput("")}>
      <div className="qr-grid"><div><label className="field-label"><span>{isTr ? "Metin veya URL" : "Text or URL"}</span><textarea rows={10} maxLength={2000} value={input} onChange={(event) => setInput(event.target.value)} /></label><p className="privacy-hint">{isTr ? `${input.length}/2.000 karakter · İçerik QR servisine gönderilmez.` : `${input.length}/2,000 characters · Content is not sent to a QR service.`}</p>{error && <p className="tool-message error">{error}</p>}</div><div className="qr-result"><div className="qr-canvas">{png ? <img src={png} alt={isTr ? "Üretilen QR kod" : "Generated QR code"} /> : <span>{isTr ? "QR kod burada görünecek." : "The QR code will appear here."}</span>}</div><div className="qr-actions"><button className="primary-button" type="button" disabled={!png} onClick={() => download(png, "bytequant-qr.png")}>PNG {isTr ? "indir" : "download"}</button><button className="secondary-button" type="button" disabled={!svg} onClick={() => download(`data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`, "bytequant-qr.svg")}>SVG {isTr ? "indir" : "download"}</button></div><p>{isTr ? "Paylaşmadan veya basmadan önce kodu farklı bir cihazla test edin." : "Test the code with a different device before sharing or printing."}</p></div></div>
    </WorkbenchFrame>
  );
}
...
function ExifWorkbench({ locale }: { locale: Locale }) {
  const isTr = locale === "tr";
  const [file, setFile] = useState<File | null>(null); const [preview, setPreview] = useState(""); const [cleaned, setCleaned] = useState(""); const [metadata, setMetadata] = useState<MetadataItem[]>([]); const [dimensions, setDimensions] = useState(""); const [message, setMessage] = useState(""); const [busy, setBusy] = useState(false);
  useEffect(() => () => { if (preview) URL.revokeObjectURL(preview); if (cleaned) URL.revokeObjectURL(cleaned); }, [preview, cleaned]);
  async function choose(next: File | null) {
    if (preview) URL.revokeObjectURL(preview); if (cleaned) URL.revokeObjectURL(cleaned); setPreview(""); setCleaned(""); setMetadata([]); setDimensions(""); setMessage(""); setFile(null);
    if (!next) return;
    if (!["image/jpeg", "image/png"].includes(next.type)) { setMessage(isTr ? "Yalnızca JPEG veya PNG seçin." : "Select a JPEG or PNG file."); return; }
    if (next.size > 25 * 1024 * 1024) { setMessage(isTr ? "Dosya 25 MB sınırını aşıyor." : "The file exceeds the 25 MB limit."); return; }
    ...
    catch (reason) { setMessage((isTr ? "Görsel okunamadı: " : "Could not read the image: ") + (reason instanceof Error ? reason.message : String(reason))); }
  }
  async function clean() {
    ...
    try {
      ...
      const type = file.type === "image/png" ? "image/png" : "image/jpeg"; const blob = await new Promise<Blob>((resolve, reject) => canvas.toBlob((value) => value ? resolve(value) : reject(new Error("Encoding failed")), type, type === "image/jpeg" ? 0.92 : undefined)); if (cleaned) URL.revokeObjectURL(cleaned); setCleaned(URL.createObjectURL(blob)); setMessage(isTr ? `Temiz kopya hazır: ${(blob.size / 1024).toFixed(1)} KB. Yeniden kodlama metadata alanlarını kaldırdı.` : `Clean copy ready: ${(blob.size / 1024).toFixed(1)} KB. Re-encoding removed metadata fields.`);
    } catch (reason) { setMessage((isTr ? "Temizleme tamamlanamadı: " : "Cleaning failed: ") + (reason instanceof Error ? reason.message : String(reason))); }
    finally { setBusy(false); }
  }
  function download() { if (!cleaned || !file) return; const anchor = document.createElement("a"); anchor.href = cleaned; anchor.download = `${file.name.replace(/\.[^.]+$/, "")}-clean.${file.type === "image/png" ? "png" : "jpg"}`; anchor.click(); }
  return (
    <WorkbenchFrame locale={locale} onClear={() => choose(null)}>
      <div className="exif-grid"><div className="file-drop"><label><input type="file" accept="image/jpeg,image/png" onChange={(event) => choose(event.target.files?.[0] ?? null)} /><span>＋</span><strong>{isTr ? "JPEG veya PNG seçin" : "Choose a JPEG or PNG"}</strong><small>{isTr ? "En fazla 25 MB · Dosya ağ üzerinden gönderilmez" : "Up to 25 MB · The file is not sent over the network"}</small></label>{preview && <img src={preview} alt={isTr ? "Seçilen görsel önizlemesi" : "Selected image preview"} />}</div><div className="metadata-panel"><div className="result-header"><span>{isTr ? "Bulunan metadata" : "Detected metadata"}</span>{file && <small>{dimensions} · {(file.size / 1024).toFixed(1)} KB</small>}</div>{file ? metadata.length ? <dl>{metadata.map((item, index) => <div key={`${item.label}-${index}`}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl> : <p>{isTr ? "Okunabilir EXIF, GPS, IPTC veya PNG metin alanı bulunamadı. Temiz kopya yine de piksel verisinden yeniden oluşturulabilir." : "No readable EXIF, GPS, IPTC, or PNG text field was found. A clean copy can still be rebuilt from pixel data."}</p> : <p>{isTr ? "Dosya seçtiğinizde metadata burada listelenir." : "Metadata will be listed here after you choose a file."}</p>}<div className="exif-actions"><button type="button" className="primary-button" disabled={!file || busy} onClick={clean}>{busy ? (isTr ? "Temizleniyor…" : "Cleaning…") : (isTr ? "Temiz kopya oluştur" : "Create clean copy")}</button><button type="button" className="secondary-button" disabled={!cleaned} onClick={download}>{isTr ? "Temiz görseli indir" : "Download clean image"}</button></div>{message && <p className={message.includes("tamamlanamadı") || message.includes("failed") || message.includes("okunamadı") ? "tool-message error" : "tool-message"}>{message}</p>}<p className="privacy-hint">{isTr ? "Not: JPEG yeniden kodlama küçük kalite/boyut farkı oluşturabilir. Animasyonlu veya özel renk profilli dosyalar için profesyonel doğrulama yapın." : "Note: JPEG re-encoding can slightly change quality or size. Professionally verify animated or color-managed assets."}</p></div></div>
    </WorkbenchFrame>
  );
}
