...
export function AgentVisualStudioLoader({ locale }: { locale: Locale }) {
  const [open, setOpen] = useState(false);
  const c = copy[locale];
  if (open) return <Suspense fallback={<div className="agent-visual-loading" role="status"><span /><strong>{c.loading}</strong></div>}><AgentVisualStudio locale={locale} /></Suspense>;
  return <div className="agent-visual-gate"><div><span className="kicker">BYTEQUANT VISUAL</span><h2>{c.title}</h2><p>{c.text}</p></div><button className="primary-button" type="button" onClick={() => setOpen(true)}><span aria-hidden="true">◫</span>{c.open}<i aria-hidden="true">→</i></button></div>;
}
