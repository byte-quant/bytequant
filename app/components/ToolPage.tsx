...
export function ToolPage({ tool, locale }: { tool: Tool; locale: Locale }) {
  ...
  const directPosts = posts.filter((post) => post.relatedTools.includes(tool.slug)).slice().reverse();
  const categoryPosts = posts.filter((post) => post.relatedTools.some((slug) => getTool(slug)?.category === tool.category)).slice().reverse();
  const relatedPosts = [...directPosts, ...categoryPosts.filter((post) => !directPosts.some((direct) => direct.slug === post.slug))].slice(0, 2);
  const directLocalizedGuides = localizedGuides.filter((guide) => guide.relatedTools.includes(tool.slug)).slice().reverse();
  const categoryLocalizedGuides = localizedGuides.filter((guide) => guide.relatedTools.some((slug) => getTool(slug)?.category === tool.category)).slice().reverse();
  const recommendedLocalizedGuides = [...directLocalizedGuides, ...categoryLocalizedGuides.filter((guide) => !directLocalizedGuides.some((direct) => direct.slug === guide.slug))].slice(0, 2);
