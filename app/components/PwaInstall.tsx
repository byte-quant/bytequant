...
export function PwaProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!guideLocale) return;
    const returnTarget = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    const focusFrame = requestAnimationFrame(() => closeButtonRef.current?.focus());
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") setGuideLocale(null);
    };
  ...
  const value: PwaContextValue = { state, install };
  ...
  return <PwaContext.Provider value={value}>{children}{labels && <div className="pwa-guide-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setGuideLocale(null); }}><section className="pwa-guide-dialog" role="dialog" aria-modal="true" aria-labelledby="pwa-guide-title" aria-describedby="pwa-guide-description"><header><div><span>{labels.kicker}</span><h2 id="pwa-guide-title">{labels.guideTitle}</h2></div><button ref={closeButtonRef} data-pwa-close type="button" onClick={() => setGuideLocale(null)} aria-label={labels.close}>×</button></header><p id="pwa-guide-description">{state === "error" ? labels.failed : labels.guideIntro}</p><ol>{steps.map((step, index) => <li key={step}><span>{index + 1}</span><strong>{step}</strong></li>)}</ol><div className="pwa-guide-privacy"><span aria-hidden="true">✓</span><p>{labels.privacy}</p></div><button type="button" className="primary-button" onClick={() => setGuideLocale(null)}>{labels.close}</button></section></div>}</PwaContext.Provider>;
}
...
export function PwaInstall({ locale, compact = false }: { locale: Locale; compact?: boolean }) {
  ...
  const actionLabel = installed ? labels.installed : state === "ready" ? labels.install : labels.options;
  if (compact) return <button type="button" className="install-trigger" data-pwa-state={state} aria-label={actionLabel} title={actionLabel} onClick={() => void install(locale)} disabled={installed}><span aria-hidden="true">⇩</span><b>{actionLabel}</b></button>;
  const status = state === "checking" ? labels.checking : state === "ready" ? labels.ready : state === "error" ? labels.failed : state === "installed" ? labels.installed : labels.manualStatus;
  return <section className="section install-section" aria-labelledby={`install-${locale}`} data-pwa-state={state} data-pwa-install-mode={state === "ready" ? "native" : "guide"}><div className="container install-card"><div className="install-icon" aria-hidden="true"><span>BQ</span><i>＋</i></div><div><span className="kicker">{labels.kicker}</span><h2 id={`install-${locale}`}>{labels.title}</h2><p>{labels.body}</p><ul><li>✓ {locale === "tr" ? "Tek dokunuşla açılır" : locale === "de" ? "Mit einem Tipp öffnen" : locale === "zh" ? "一键打开" : "Open in one tap"}</li><li>✓ {locale === "tr" ? "Güvenli tarayıcı kurulumu" : locale === "de" ? "Sichere Browser-Installation" : locale === "zh" ? "安全的浏览器安装" : "Safe browser installation"}</li><li>✓ {locale === "tr" ? "Girdiler önbelleğe alınmaz" : locale === "de" ? "Eingaben werden nicht gecacht" : locale === "zh" ? "输入不会缓存" : "Inputs are never cached"}</li></ul></div><div className="install-actions" aria-live="polite" aria-atomic="true"><button type="button" className="primary-button" onClick={() => void install(locale)} disabled={installed}>{actionLabel}</button><small>{status}</small></div></div></section>;
}
