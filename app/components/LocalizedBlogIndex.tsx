...
export function LocalizedBlogIndex({ locale }: { locale: "de" | "zh" }) {
  ...
  return (
    <SiteShell locale={locale} alternateHref="/en/blog" languageHrefs={{ tr: "/blog", en: "/en/blog", de: "/de/blog", zh: "/zh/blog" }}>
      ...
      <section className="page-hero"><div className="container narrow-container"><nav className="breadcrumbs" aria-label={isDe ? "Brotkrumen" : "面包屑导航"}><Link href={pathFor(locale, "home")}>{isDe ? "Startseite" : "首页"}</Link><span aria-hidden="true">/</span><span aria-current="page">{isDe ? "Ratgeber" : "指南"}</span></nav><span className="kicker">{isDe ? "BYTEQUANT RATGEBER" : "BYTEQUANT 指南"}</span><h1>{isDe ? "Methoden, Grenzen und sichere Arbeitsabläufe" : "方法、限制与安全工作流程"}</h1><p>{isDe ? "Neue Arbeitsabläufe werden redaktionell auf Deutsch erklärt. Weitere etablierte Fachartikel bleiben als transparent gekennzeichnete englische Originale zugänglich." : "新的工作流已完成中文编辑本地化；其他成熟专业文章仍以清晰标注的英文原文提供。"}</p><a className="text-link" href={locale === "de" ? "/de/feed.xml" : "/zh/feed.xml"} aria-label={isDe ? "Ratgeber-RSS-Feed öffnen" : "打开指南 RSS 订阅源"}>RSS ↗</a></div></section>
