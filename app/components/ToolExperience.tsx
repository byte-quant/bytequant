...
export function ToolExperience({ slug, locale, related, compare }: { slug: string; locale: Locale; related: RelatedTool[]; compare: boolean }) {
  ...
  return <>
    ...
    <section className="tool-next-actions"><div><span className="kicker">{t.nextKicker}</span><h2>{t.next}</h2><p>{t.bridge}</p></div><div className="tool-transfer-control"><label><span>{t.choose}</span><select value={target} onChange={(event) => setTarget(event.target.value)}>{related.map((item) => <option key={item.slug} value={item.slug}>{item.title}</option>)}</select></label><button type="button" className="primary-button" onClick={transfer}>{t.transfer} →</button><button type="button" className="secondary-button" onClick={transferToWorkspace}>{t.flow} ↗</button></div><nav aria-label={t.frequent}><strong>{t.frequent}</strong>{related.map((item) => <Link key={item.slug} href={toolPath(locale, item.slug)}>{item.title} →</Link>)}</nav></section>
  </>;
}
