...
export function ToolExperience({ slug, title, locale, related, compare }: { slug: string; title: string; locale: Locale; related: RelatedTool[]; compare: boolean }) {
  ...
  return <>
    <section className="tool-experience-bar" aria-label={t.status}>
      ...
      <div className="tool-version-badges"><span>✓ {t.local}</span><time dateTime={CONTENT_REVIEW_DATE}>{t.updated}</time></div>
    ...
    {compare && (before || after) && <section className="before-after-panel"><header><div><span className="kicker">{t.compareKicker}</span><h2>{t.compare}</h2></div><button type="button" onClick={refreshComparison}>{t.refresh}</button></header><div><article><strong>{t.before}</strong><StructuredToolOutput output={before} empty="—" compact /></article><article><strong>{t.after}</strong><StructuredToolOutput output={after} empty="—" compact /></article></div></section>}
