...
import { WORKSPACE_TOOL_START_KEY, type WorkspaceToolStart } from "../lib/workspace-handoff";
...
const copy = {
  tr: { ready: "Hazır", processing: "İşleniyor", complete: "Tamamlandı", error: "Kontrol gerekiyor", status: "İşlem durumu", favorite: "Sık kullanılanlara sabitle", unfavorite: "Sabitlenenlerden çıkar", consent: "Favoriler için yerel kişiselleştirmeyi açın.", updated: "Sayfa güncellemesi:", local: "Tamamen tarayıcıda çalışır", compareKicker: "SONUCU KARŞILAŞTIR", nextKicker: "SIRADAKİ ADIM", next: "Bu sonucu şimdi başka bir araçta işleyin", choose: "Sonraki aracı seçin", transfer: "Sonucu aktar", flow: "Görsel akışta devam et", noOutput: "Aktarılabilecek bir çıktı henüz yok.", transferred: "Önceki araçtaki çıktı güvenli sekme içi aktarım ile yüklendi.", privacy: "Bu işlem tamamen cihazınızda gerçekleşti", privacyBody: "Girdi ve çıktı ByteQuant sunucusuna gönderilmedi. İndirme veya başka araca aktarım yalnızca sizin eyleminizle yapılır.", compare: "Önce / Sonra", before: "Önce", after: "Sonra", refresh: "Karşılaştırmayı güncelle", frequent: "Bu araçla sık kullanılanlar", bridge: "Sonuç bu sekmede kısa süre tutulur; doğrudan sonraki araca geçebilir veya daha uzun bir görsel akış kurabilirsiniz." },
  en: { ready: "Ready", processing: "Processing", complete: "Completed", error: "Review needed", status: "Operation status", favorite: "Pin to favorites", unfavorite: "Remove from favorites", consent: "Enable local personalization to use favorites.", updated: "Page updated:", local: "Runs entirely in your browser", compareKicker: "COMPARE THE RESULT", nextKicker: "NEXT STEP", next: "Process this result with another tool", choose: "Choose the next tool", transfer: "Transfer result", flow: "Continue in a visual flow", noOutput: "There is no transferable output yet.", transferred: "Output from the previous tool was loaded through a safe same-tab handoff.", privacy: "This operation happened entirely on your device", privacyBody: "Input and output were not sent to a ByteQuant server. Download or transfer happens only when you choose it.", compare: "Before / After", before: "Before", after: "After", refresh: "Refresh comparison", frequent: "Frequently used with this tool", bridge: "The result stays briefly in this tab; continue directly to the next tool or build a longer visual flow." },
  de: { ready: "Bereit", processing: "Verarbeitung", complete: "Abgeschlossen", error: "Prüfung nötig", status: "Vorgangsstatus", favorite: "Zu Favoriten hinzufügen", unfavorite: "Aus Favoriten entfernen", consent: "Aktivieren Sie lokale Personalisierung für Favoriten.", updated: "Seite aktualisiert:", local: "Läuft vollständig im Browser", compareKicker: "ERGEBNIS VERGLEICHEN", nextKicker: "NÄCHSTER SCHRITT", next: "Ergebnis mit einem weiteren Werkzeug verarbeiten", choose: "Nächstes Werkzeug wählen", transfer: "Ergebnis übergeben", flow: "Im visuellen Ablauf fortfahren", noOutput: "Noch keine übertragbare Ausgabe vorhanden.", transferred: "Die vorherige Ausgabe wurde über eine sichere Übergabe im selben Tab geladen.", privacy: "Dieser Vorgang fand vollständig auf Ihrem Gerät statt", privacyBody: "Ein- und Ausgabe wurden nicht an ByteQuant gesendet. Download oder Übergabe erfolgen nur durch Ihre Aktion.", compare: "Vorher / Nachher", before: "Vorher", after: "Nachher", refresh: "Vergleich aktualisieren", frequent: "Häufig zusammen verwendet", bridge: "Das Ergebnis bleibt kurz in diesem Tab; direkt zum nächsten Werkzeug wechseln oder einen längeren visuellen Ablauf erstellen." },
  zh: { ready: "就绪", processing: "处理中", complete: "已完成", error: "需要检查", status: "处理状态", favorite: "固定到收藏", unfavorite: "从收藏移除", consent: "请启用本地个性化以使用收藏。", updated: "页面更新：", local: "完全在浏览器内运行", compareKicker: "对比结果", nextKicker: "下一步", next: "将此结果交给另一个工具继续处理", choose: "选择下一个工具", transfer: "传递结果", flow: "在可视化流程中继续", noOutput: "目前没有可传递的输出。", transferred: "上一工具的输出已通过同一标签页内的安全交接载入。", privacy: "此处理完全在您的设备上完成", privacyBody: "输入与输出未发送到 ByteQuant 服务器；下载或传递只会由您的操作触发。", compare: "处理前 / 处理后", before: "处理前", after: "处理后", refresh: "更新对比", frequent: "常与此工具搭配使用", bridge: "结果会在当前标签页短暂保留；可直接转到下一工具，也可建立更长的可视化流程。" },
} as const;
...
export function ToolExperience({ slug, title, locale, related, compare }: { slug: string; title: string; locale: Locale; related: RelatedTool[]; compare: boolean }) {
  const t = copy[locale];
  ...
  useEffect(() => {
    ...
    const inspect = () => {
      if (scope.querySelector(".tool-notice.error, [role=alert]")) { setState("error"); setBefore(""); setAfter(""); return; }
    ...
    const onInput = () => { setState("ready"); setBefore(""); setAfter(""); };
  ...
  function transferToWorkspace() {
    const value = readableOutput(document.querySelector<HTMLElement>(".workbench"));
    if (!value || !target || !relatedMap.has(target)) { setNotice(t.noOutput); return; }
    const record: WorkspaceToolStart = { version: 1, toolSlug: target, input: value, locale, createdAt: Date.now() };
    try { sessionStorage.setItem(WORKSPACE_TOOL_START_KEY, JSON.stringify(record)); window.location.assign(pathFor(locale, "workstation")); }
    catch { setNotice(t.noOutput); }
  }
  ...
  return <>
    <section className="tool-experience-bar" aria-label={t.status}>
      ...
      <div className="tool-version-badges"><span>✓ {t.local}</span><time dateTime={TOOL_PAGE_UPDATED}>{t.updated} {new Intl.DateTimeFormat(locale === "zh" ? "zh-CN" : locale, { dateStyle: "long", timeZone: "UTC" }).format(new Date(`${TOOL_PAGE_UPDATED}T00:00:00Z`))}</time></div>
    ...
    {compare && state === "complete" && (before || after) && <section className="before-after-panel"><header><div><span className="kicker">{t.compareKicker}</span><h2>{t.compare}</h2></div><button type="button" onClick={refreshComparison}>{t.refresh}</button></header><div><article><strong>{t.before}</strong><StructuredToolOutput output={before} empty="—" compact /></article><article><strong>{t.after}</strong><StructuredToolOutput output={after} empty="—" compact /></article></div></section>}
    <section className="tool-next-actions"><div><span className="kicker">{t.nextKicker}</span><h2>{t.next}</h2><p>{({ tr: `${title} çıktısı bu sekmede kısa süre tutulur. ${relatedMap.get(target)?.title ?? related[0]?.title ?? "Seçtiğiniz araç"} ile devam edebilir veya daha uzun bir görsel akış kurabilirsiniz.`, en: `${title} output stays briefly in this tab. Continue with ${relatedMap.get(target)?.title ?? related[0]?.title ?? "your selected tool"}, or build a longer visual flow.`, de: `Die Ausgabe von ${title} bleibt kurz in diesem Tab. Fahren Sie mit ${relatedMap.get(target)?.title ?? related[0]?.title ?? "dem ausgewählten Werkzeug"} fort oder erstellen Sie einen längeren visuellen Ablauf.`, zh: `${title}的输出会在当前标签页短暂保留。您可以继续使用${relatedMap.get(target)?.title ?? related[0]?.title ?? "所选工具"}，或建立更长的可视化流程。` } as const)[locale]}</p></div><div className="tool-transfer-control"><label><span>{t.choose}</span><select value={target} onChange={(event) => setTarget(event.target.value)}>{related.map((item) => <option key={item.slug} value={item.slug}>{item.title}</option>)}</select></label><button type="button" className="primary-button" onClick={transfer}>{t.transfer} →</button><button type="button" className="secondary-button" onClick={transferToWorkspace}>{t.flow} ↗</button></div><nav aria-label={t.frequent}><strong>{t.frequent}</strong>{related.map((item) => <Link key={item.slug} href={toolPath(locale, item.slug)}>{item.title} →</Link>)}</nav></section>
  </>;
}
