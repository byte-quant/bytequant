...
type CommunityAudience = "public" | "private" | "group";
type ShareDraft = { version: 1; kind: CommunityKind; title: string; body: string; recipe: string; audience: CommunityAudience; group: string };
...
const audienceCopy = {
  ...
  en: { label: "Who can see it?", public: "Public through a content link", private: "Private on this device", group: "Private group board", groupName: "Group name", privateNote: "Private posts are excluded from share links and public board packs." },
  de: { label: "Wer kann es sehen?", public: "Öffentlich über Inhaltslink", private: "Privat auf diesem Gerät", group: "Privates Gruppenboard", groupName: "Gruppenname", privateNote: "Private Beiträge werden nicht in Teilungslinks oder öffentliche Pakete aufgenommen." },
  zh: { label: "谁可以查看？", public: "通过内容链接公开", private: "仅本设备私密", group: "私密群组看板", groupName: "群组名称", privateNote: "私密帖子不会进入分享链接或公开社区包。" },
} as const;
const boardCopy = {
...
const composerUi = {
  tr: { edit: "1 · Taslağınızı hazırlayın", editHelp: "Hazır bir fikir seçin veya alanları kendi sözlerinizle doldurun.", preview: "2 · Paylaşmadan önce görünüm", previewHelp: "Başlık, hedef kitle ve metin burada okunabilir bir kart olarak görünür.", emptyTitle: "Başlığınız burada görünecek", emptyBody: "Kısa açıklamanızı yazdığınızda paylaşım önizlemesi burada oluşacak.", locked: "Önce cihaz içi kontrolü tamamlayın; paylaşım seçenekleri sonra açılır.", recipe: "Bağlı iş akışı" },
  en: { edit: "1 · Prepare your draft", editHelp: "Choose a starter or complete the fields in your own words.", preview: "2 · Preview before sharing", previewHelp: "Title, audience, and text appear here as a readable post card.", emptyTitle: "Your title will appear here", emptyBody: "Write a short description to build the post preview.", locked: "Complete the on-device check first; sharing options will then unlock.", recipe: "Linked workflow" },
  de: { edit: "1 · Entwurf vorbereiten", editHelp: "Vorlage wählen oder die Felder mit eigenen Worten ausfüllen.", preview: "2 · Vorschau vor dem Teilen", previewHelp: "Titel, Zielgruppe und Text erscheinen hier als lesbare Beitragskarte.", emptyTitle: "Ihr Titel erscheint hier", emptyBody: "Eine kurze Beschreibung erzeugt die Beitragsvorschau.", locked: "Zuerst die lokale Prüfung abschließen; danach werden die Teilungsoptionen freigeschaltet.", recipe: "Verknüpfter Ablauf" },
  zh: { edit: "1 · 准备草稿", editHelp: "选择一个模板，或用自己的文字填写各字段。", preview: "2 · 分享前预览", previewHelp: "标题、受众和正文会以易读的帖子卡片显示在这里。", emptyTitle: "标题会显示在这里", emptyBody: "填写简短说明后，这里会生成帖子预览。", locked: "请先完成设备端检查，随后会解锁分享选项。", recipe: "关联工作流" },
} as const;
...
function decodeDraft(value: string): ShareDraft | null {
  try {
    ...
    const audience = ["public", "private", "group"].includes(draft.audience ?? "") ? draft.audience as CommunityAudience : "public";
    return { version: 1, kind: draft.kind as CommunityKind, title: clean(draft.title, 120), body: clean(draft.body, 5000), recipe: clean(draft.recipe, 4000), audience, group: clean(draft.group ?? "", 50) };
  } catch { return null; }
}

export function CommunityComposer({ locale }: { locale: Locale }) {
  const t = copy[locale]; const issue = issueCopy[locale]; const audienceText = audienceCopy[locale]; const composer = composerUi[locale];
  const [kind, setKind] = useState<CommunityKind>("workflow"); const [audience, setAudience] = useState<CommunityAudience>("public"); const [group, setGroup] = useState(""); const [title, setTitle] = useState(""); const [body, setBody] = useState(""); const [recipe, setRecipe] = useState(""); const [checked, setChecked] = useState(false); const [status, setStatus] = useState("");
  const issues = useMemo(() => {
    ...
    if (audience === "group" && group.trim().length < 3) result.push(`${audienceText.groupName}: 3+`);
    return [...new Set(result)];
  }, [audience, audienceText.groupName, body, group, issue, recipe, t.empty, t.safeRecipe, title]);
  const markdown = useMemo(() => `## ${clean(title, 120)}\n\n**Type:** ${t.kinds[kind]}\n**Audience:** ${audience === "group" ? `${audienceText.group}: ${clean(group, 50)}` : audienceText[audience]}\n\n${clean(body, 5000)}${recipe.trim() ? `\n\n**ByteQuant recipe:** ${clean(recipe, 4000)}` : ""}\n\n---\nPrivacy check: no intentional personal data or secrets included.`, [audience, audienceText, body, group, kind, recipe, t.kinds, title]);
  const canShare = checked && issues.length === 0;
  const draft = (): ShareDraft => ({ version: 1, kind, title: clean(title, 120), body: clean(body, 5000), recipe: clean(recipe, 4000), audience, group: clean(group, 50) });
  ...
  async function shareLink() { if (!canShare || audience !== "public") { setStatus(audienceText.privateNote); return; } const url = new URL(window.location.href); url.searchParams.set("share", encodeDraft(draft())); await write(url.toString(), t.linked); }
  async function nativeShare() { if (!canShare || audience !== "public") { setStatus(audienceText.privateNote); return; } const url = new URL(window.location.href); url.searchParams.set("share", encodeDraft(draft())); if (navigator.share) { try { await navigator.share({ title, text: body.slice(0, 180), url: url.toString() }); return; } catch { return; } } await write(url.toString(), t.linked); }
  ...
  function download() { if (!canShare) return; const blob = new Blob([markdown], { type: "text/markdown;charset=utf-8" }); const href = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = href; anchor.download = `${title.toLocaleLowerCase().replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "bytequant-share"}.md`; anchor.click(); window.setTimeout(() => URL.revokeObjectURL(href), 0); }
  function importFromLink() { const value = new URL(window.location.href).searchParams.get("share"); const imported = value ? decodeDraft(value) : null; if (!imported) { setStatus(t.noDraft); return; } setKind(imported.kind); setAudience(imported.audience); setGroup(imported.group); setTitle(imported.title); setBody(imported.body); setRecipe(imported.recipe); setChecked(false); setStatus(t.imported); }
  function applyTemplate(index: number) { const value = templates[locale][index]; setKind(value.kind); setAudience("public"); setGroup(""); setTitle(value.title); setBody(value.body); setRecipe(""); setChecked(false); setStatus(""); }
  return <div className="community-composer">
    <div className="community-form">
      <header className="community-editor-heading"><span>1</span><div><strong>{composer.edit}</strong><small>{composer.editHelp}</small></div></header>
      <p>{t.intro}</p>
      <fieldset className="community-templates"><legend>{t.templates}</legend>{t.templateNames.map((name, index) => <button type="button" key={name} onClick={() => applyTemplate(index)}>{name}</button>)}</fieldset>
      <div className="community-composer-row"><label><span>{t.kind}</span><select value={kind} onChange={(event) => { setKind(event.target.value as CommunityKind); setChecked(false); }}>{(Object.keys(t.kinds) as CommunityKind[]).map((key) => <option value={key} key={key}>{t.kinds[key]}</option>)}</select></label><label><span>{audienceText.label}</span><select value={audience} onChange={(event) => { setAudience(event.target.value as CommunityAudience); setChecked(false); }}><option value="public">{audienceText.public}</option><option value="private">{audienceText.private}</option><option value="group">{audienceText.group}</option></select></label></div>
      {audience === "group" && <label><span>{audienceText.groupName}</span><input value={group} maxLength={50} onChange={(event) => { setGroup(event.target.value); setChecked(false); }} /></label>}
      <small className="community-privacy-explainer"><span aria-hidden="true">●</span>{audienceText.privateNote}</small>
      <label><span>{t.title}</span><input value={title} maxLength={120} onChange={(event) => { setTitle(event.target.value); setChecked(false); }} /></label>
      <label><span>{t.body}</span><textarea value={body} maxLength={5000} rows={7} onChange={(event) => { setBody(event.target.value); setChecked(false); }} /></label>
      <label><span>{t.recipe}</span><input type="url" value={recipe} maxLength={4000} onChange={(event) => { setRecipe(event.target.value); setChecked(false); }} placeholder="https://bytequant.org/workspace?recipe=…" /></label>
      <div className="community-form-actions"><button type="button" className="primary-button" onClick={() => { setChecked(true); setStatus(""); }}>{t.check}</button><button type="button" className="secondary-button" onClick={importFromLink}>{t.import}</button></div>
      <small className="community-moderation-boundary">{t.boundary}</small>{status && <p className="community-status" role="status">{status}</p>}
    </div>
    <aside className={checked && issues.length ? "community-preview blocked" : "community-preview"}>
      <header className="community-editor-heading"><span>{checked ? issues.length ? "!" : "✓" : "2"}</span><div><strong>{composer.preview}</strong><small>{checked ? issues.length ? t.blocked : t.safe : composer.previewHelp}</small></div></header>
      <article className="community-draft-card">
        <div className="community-draft-meta"><span>{t.kinds[kind]}</span><span>{audience === "group" ? `${audienceText.group}: ${clean(group, 50) || "—"}` : audienceText[audience]}</span></div>
        <h3>{clean(title, 120) || composer.emptyTitle}</h3>
        <p>{clean(body, 5000) || composer.emptyBody}</p>
        {recipe.trim() && <div className="community-draft-recipe"><strong>{composer.recipe}</strong><code>{clean(recipe, 4000)}</code></div>}
      </article>
      {checked && issues.length > 0 && <section className="community-preview-issues"><h3>{t.issues}</h3><ul>{issues.map((item) => <li key={item}>{item}</li>)}</ul></section>}
      {canShare ? <div className="community-preview-actions"><button type="button" className="community-board-action" onClick={addToBoard}>{boardCopy[locale].action}</button><button type="button" onClick={() => write(markdown, t.copied)}>{t.copy}</button><button type="button" onClick={shareLink} disabled={audience !== "public"}>{t.link}</button><button type="button" onClick={nativeShare} disabled={audience !== "public"}>{t.share}</button><button type="button" onClick={download}>{t.download}</button><a className="secondary-button" href="https://github.com/byte-quant/bytequant/discussions" target="_blank" rel="noreferrer noopener">{t.discuss} ↗</a></div> : <p className="community-preview-locked"><span aria-hidden="true">○</span>{composer.locked}</p>}
    </aside>
  </div>;
}
