    ...
    <SiteShell locale={locale} alternateHref={alternateHref} languageHrefs={{ tr: "/", en: "/en", de: "/de", zh: "/zh" }}>
      ...
      <section className="section category-section" id={locale === "tr" ? "araclar" : "tools"}>
        <div className="container">
          <div className="section-heading split-heading"><div><span className="kicker">{localized("ARAÇ KÜTÜPHANESİ", "TOOL LIBRARY", "WERKZEUGBIBLIOTHEK", "工具库")}</span><h2>{localized("Günlük işler için küçük, güvenilir araçlar", "Small, dependable tools for everyday work", "Kleine, verlässliche Werkzeuge für den Alltag", "适合日常工作的可靠小工具")}</h2></div><p>{localized("Her araç tek bir işi iyi yapar, yöntemi açıklar ve veriyi dışarı göndermeden kullanılabilir bir sonuç üretir.", "Each tool does one job well, explains its method, and produces a useful result without sending input away.", "Jedes Werkzeug löst eine Aufgabe, erklärt seine Methode und erzeugt eine nutzbare Ausgabe, ohne Eingaben zu versenden.", "每个工具专注解决一项任务，说明方法，并在不发送输入的情况下生成可用结果。")}</p></div>
      ...
      <section className="section editorial-section">
        <div className="container"><div className="section-heading split-heading"><div><span className="kicker">{localized("REHBERLER", "GUIDES", "RATGEBER", "指南")}</span><h2>{localized("Aracı değil, doğru yöntemi öğrenin", "Learn the method, not only the tool", "Lernen Sie die Methode, nicht nur das Werkzeug", "学习方法，而不只是工具")}</h2></div><p>{localized("Gizlilik, prompt kalitesi ve veri güvenliği hakkında uygulamaya dönük, kaynak mantığı açık yazılar.", "Practical writing on privacy, prompt quality, and data security with transparent reasoning.", "Praxisnahe Inhalte zu Datenschutz, Prompt-Qualität und Datensicherheit mit nachvollziehbarer Begründung.", "关于隐私、提示词质量和数据安全的实用内容，并提供透明依据。")}</p></div><div className="post-grid">{posts.slice(0, 3).map((post, index) => <article className={`post-card post-card-${index + 1}`} key={post.slug}><span>{post.category[editorialLocale]} · {post.readTime[editorialLocale]}</span><h3><Link href={postPath(editorialLocale, post.slug)}>{post.title[editorialLocale]}</Link></h3><p>{post.excerpt[editorialLocale]}</p><Link className="text-link" href={postPath(editorialLocale, post.slug)}>{localized("Rehberi oku", "Read guide", "Ratgeber auf Englisch lesen", "阅读英文指南")} →</Link></article>)}</div><div className="center-action"><Link className="secondary-button" href={pathFor(locale, "blog")}>{localized("Tüm rehberleri görüntüle", "View all guides", "Alle Ratgeber", "查看全部指南")}</Link></div></div>
      </section>
