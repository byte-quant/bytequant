...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return (
    <SiteShell locale={locale} alternateHref={alternateHref} languageHrefs={{ tr: "/", en: "/en", de: "/de", zh: "/zh" }}>
      ...
      <section className="hero-section">
        <div className="container hero-grid">
          <div className="hero-copy">
            ...
            <h1>{localized("İşinizi kolaylaştırın; hassas veriniz sizde kalsın.", "Make the task easier while sensitive data stays with you.", "Aufgaben leichter erledigen – sensible Daten bleiben bei Ihnen.", "让任务更轻松，同时让敏感数据留在您手中。")}</h1>
            <p>{localized(`JSON düzeltmekten PDF birleştirmeye, prompt geliştirmekten kişisel veri temizlemeye kadar ${tools.length} araç yanınızda. Üyelik yok; çekirdek işlemler etkin tarayıcı sekmesinde çalışır.`, `From fixing JSON and merging PDFs to improving prompts and cleaning personal data, ${tools.length} tools are ready. No account is required, and core processing runs in the active browser tab.`, `Von JSON und PDF bis zu Prompts und Datenschutz: ${tools.length} Werkzeuge stehen bereit. Kein Konto nötig; die Kernverarbeitung läuft im aktiven Browser-Tab.`, `从修复 JSON、合并 PDF 到优化提示词和清理个人数据，${tools.length} 个工具随时可用。无需账号，核心处理在当前浏览器标签页完成。`)}</p>
            <HeroToolSearch locale={locale} />
            <div className="hero-actions"><Link className="primary-button" href={pathFor(locale, "agent")}>{localized("Ne yapacağımı anlatayım", "Describe what I need", "Mein Ziel beschreiben", "描述我的需求")}<span aria-hidden="true"> →</span></Link><Link className="secondary-button" href={pathFor(locale, "tools")}>{t.allTools}</Link><Link className="hero-workstation-link" href={pathFor(locale, "workstation")}>⌘ {localized("Akış kur", "Build a flow", "Ablauf bauen", "建立流程")}</Link></div>
            ...
            <div className="hero-use-cases" aria-label={localized("Popüler başlangıçlar", "Popular starting points", "Beliebte Einstiege", "热门起点")}><small>{localized("Hemen deneyin", "Try now", "Direkt starten", "立即试用")}</small><div><Link href={toolPath(locale, "json-bicimlendirici")}>JSON</Link><Link href={toolPath(locale, "pdf-birlestirme")}>PDF</Link><Link href={toolPath(locale, "kvkk-veri-maskeleyici")}>{localized("Veri maskele", "Mask data", "Daten maskieren", "数据遮蔽")}</Link><Link href={toolPath(locale, "gorsel-sikistirici")}>{localized("Görsel küçült", "Compress image", "Bild komprimieren", "图片压缩")}</Link></div></div>
          </div>
      ...
      <section className="home-social-strip"><div className="container"><div><span aria-hidden="true">◎</span><p><strong>{localized("Yönteminizi paylaşın", "Share a method", "Methode teilen", "分享方法")}</strong>{localized("Hesapsız topluluk panosunda güvenli bir tarif yayınlayın veya içerikli bağlantı gönderin.", "Publish a safe recipe on the no-account community board or send a content-carrying link.", "Sicheres Rezept ohne Konto veröffentlichen oder als Inhaltslink teilen.", "在免账号社区看板发布安全配方，或发送包含内容的链接。")}</p><Link href={pathFor(locale, "community")}>{localized("Topluluğu aç", "Open community", "Community öffnen", "打开社区")} →</Link></div><div><span aria-hidden="true">◉</span><p><strong>{localized("Kaynağıyla gündem", "Updates with sources", "Aktuell mit Quellen", "带来源的动态")}</strong>{localized("Bilim, AI ve güvenlik başlıklarını resmî kaynağa giderek doğrulayın.", "Follow science, AI, and security headlines back to official sources.", "Wissenschaft, KI und Sicherheit direkt an offiziellen Quellen prüfen.", "从官方来源核验科学、AI 与安全动态。")}</p><Link href={pathFor(locale, "news")}>{localized("Gündeme git", "View updates", "Aktuell öffnen", "查看动态")} →</Link></div></div></section>
