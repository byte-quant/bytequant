...
export type HomeToolSummary = { slug: string; category: ToolCategory; mark: string; title: string; short: string; categoryLabel: string };

export function HeroToolSearch({ locale, tools, placeholder }: { locale: Locale; tools: HomeToolSummary[]; placeholder?: string }) {
  const t = {
  ...
  const results = useMemo(() => {
    ...
    return tools.map((tool) => {
      const title = fold(tool.title);
      const haystack = `${title} ${fold(tool.slug)} ${fold(tool.short)} ${fold(tool.categoryLabel)}`;
    ...
    }).filter((item) => item.score >= 0).sort((a, b) => b.score - a.score || a.tool.title.localeCompare(b.tool.title)).slice(0, 7);
  }, [deferred, tools]);
