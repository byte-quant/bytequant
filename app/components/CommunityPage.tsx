...
export function CommunityPage({ locale }: { locale: Locale }) {
  ...
  return (
    <SiteShell locale={locale} alternateHref={pathFor(locale === "tr" ? "en" : "tr", "community")} languageHrefs={{ tr: pathFor("tr", "community"), en: pathFor("en", "community"), de: pathFor("de", "community"), zh: pathFor("zh", "community") }}>
      ...
      <section className="section community-compose-section" id="community-compose"><div className="container"><div className="section-heading"><span className="kicker">CREATE · REVIEW · CHOOSE AUDIENCE</span><h2>{locale === "tr" ? "Paylaşımınızı hazırlayın" : locale === "de" ? "Beitrag vorbereiten" : locale === "zh" ? "准备分享内容" : "Prepare your post"}</h2><p>{locale === "tr" ? "Önce hedef kitleyi seçin; yerel güvenlik kontrolünden geçen gönderi herkese açık, yalnızca cihazda özel veya grup içeriği olabilir." : locale === "de" ? "Zielgruppe wählen; geprüfte Beiträge können öffentlich, nur lokal oder einer Gruppe zugeordnet sein." : locale === "zh" ? "先选择受众；通过本地安全检查的帖子可设为公开、仅设备私密或群组内容。" : "Choose the audience first; a locally reviewed post can be public, on-device private, or group-only."}</p></div><CommunityComposer locale={locale} /></div></section>
      <section className="section community-feed-section"><div className="container"><CommunityFeed locale={locale} /></div></section>
      <section className="section community-live-section"><div className="container"><details className="community-live-details"><summary><span>●</span><strong>{locale === "tr" ? "İsteğe bağlı canlı P2P oturumu" : locale === "de" ? "Optionale direkte P2P-Sitzung" : locale === "zh" ? "可选的实时 P2P 会话" : "Optional live P2P session"}</strong><small>{locale === "tr" ? "Yalnızca aynı anda çevrimiçi kişiler için" : locale === "de" ? "Nur für gleichzeitig aktive Personen" : locale === "zh" ? "仅适用于同时在线的参与者" : "For people online at the same time"}</small><b>+</b></summary><CommunityP2PChat locale={locale} /></details></div></section>
