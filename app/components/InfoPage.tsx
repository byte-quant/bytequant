...
export function InfoPage({ pageKey, locale }: { pageKey: InfoKey; locale: Locale }) {
  ...
  const schema = pageKey === "faq" ? { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: content.sections[locale].map((section) => ({ "@type": "Question", name: section.heading, acceptedAnswer: { "@type": "Answer", text: section.paragraphs.join(" ") } })) } : { "@context": "https://schema.org", "@type": type, name: content.title[locale], description: content.intro[locale], url: `${siteUrl}${pathFor(locale, pageKey)}`, inLanguage: locale, isPartOf: { "@type": "WebSite", name: "ByteQuant", url: siteUrl } };
  return (
