...
import { absoluteUrl, languageTag, organizationId, pageLanguageHrefs, pathFor, websiteId } from "../lib/site";
...
export function InfoPage({ pageKey, locale }: { pageKey: InfoKey; locale: InfoLocale }) {
  ...
  return (
    <SiteShell locale={locale} alternateHref={alternateHref} languageHrefs={pageLanguageHrefs(pageKey)} includeGlobalSchema={pageKey === "about"}>
      ...
      {pageKey !== "faq" && <nav className="info-section-nav" aria-label={isTr ? "Bu sayfadaki bölümler" : "Sections on this page"}><div className="container narrow-container"><strong>{isTr ? "Bu sayfada" : "On this page"}</strong><div>{content.sections[locale].map((section, index) => <a href={`#section-${index + 1}`} key={section.heading}><span>{String(index + 1).padStart(2, "0")}</span>{section.heading}</a>)}</div></div></nav>}
