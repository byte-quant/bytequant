...
const content = {
  tr: {
    ...
    badges: ["89 araçla senkronize", "0 uzak model çağrısı", "Görünür karar özeti"],
  ...
  en: {
    ...
    badges: ["Synced with 89 tools", "0 remote model calls", "Visible decision summary"],
  ...
  de: {
    ...
    badges: ["Mit 89 Werkzeugen synchronisiert", "0 Remote-Modellaufrufe", "Sichtbare Entscheidungsübersicht"],
  ...
  zh: {
    ...
    badges: ["同步 89 个工具", "0 次远程模型调用", "决策摘要可见"],
...
export function AgentPage({ locale }: { locale: Locale }) {
  ...
  return <SiteShell locale={locale} alternateHref={pathFor(locale === "tr" ? "en" : "tr", "agent")} languageHrefs={{ tr: pathFor("tr", "agent"), en: pathFor("en", "agent"), de: pathFor("de", "agent"), zh: pathFor("zh", "agent") }}>
    ...
    <section className="agent-hero"><div className="container agent-hero-grid"><div><span className="eyebrow"><i />{c.eyebrow}</span><h1>{c.title}</h1><p>{c.intro}</p><div className="agent-badges">{c.badges.map((item) => <span key={item}>✓ {item}</span>)}</div><a className="primary-button" href="#local-agent">{c.cta} →</a></div><div className="agent-orbit" aria-hidden="true"><span className="agent-orbit-core">BQ<small>Agent</small></span>{["{}", "PDF", "AI", "CSV", "🔒", "⌘"].map((item, index) => <i key={item} style={{ "--orbit-index": index } as CSSProperties}>{item}</i>)}</div></div></section>
    ...
    <section className="section agent-privacy"><div className="container agent-privacy-grid"><div><span className="kicker">PRIVACY BY ARCHITECTURE</span><h2>{c.privacyTitle}</h2><p>bytequant.org · {siteUrl.replace("https://", "")}</p></div><ul>{c.privacy.map((item) => <li key={item}><span>✓</span><p>{item}</p></li>)}</ul></div></section>
