...
export function AgentPage({ locale }: { locale: Locale }) {
  ...
  const readingLinks = { tr: ["Bu sayfada", "Girdi–çıktı örnekleri", "Yetenekler ve sınırlar", "Sorular ve yanıtlar"], en: ["On this page", "Input–output examples", "Capabilities and limits", "Questions and answers"], de: ["Auf dieser Seite", "Ein- und Ausgabebeispiele", "Funktionen und Grenzen", "Fragen und Antworten"], zh: ["本页内容", "输入与输出示例", "能力与限制", "问题与解答"] }[locale];
  ...
  return <SiteShell locale={locale} alternateHref={pathFor(locale === "tr" ? "en" : "tr", "agent")} languageHrefs={{ tr: pathFor("tr", "agent"), en: pathFor("en", "agent"), de: pathFor("de", "agent"), zh: pathFor("zh", "agent") }}>
    ...
    <nav className="container page-reading-nav" aria-label={readingLinks[0]}><a href="#agent-worked-examples">{readingLinks[1]}</a><a href="#agent-capabilities">{readingLinks[2]}</a><a href="#agent-faq">{readingLinks[3]}</a></nav>
    ...
    <section className="section compact-faq" id="agent-faq"><div className="container"><div className="section-heading centered"><span className="kicker">FAQ</span><h2>{locale === "tr" ? "Kısa ve açık yanıtlar" : locale === "de" ? "Kurze, klare Antworten" : locale === "zh" ? "简明解答" : "Short, clear answers"}</h2></div><div className="faq-list narrow">{c.faq.map(([question, answer]) => <details key={question}><summary>{question}<span>+</span></summary><p>{answer}</p></details>)}</div></div></section>
  </SiteShell>;
}
