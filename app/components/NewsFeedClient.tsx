...
export function NewsFeedClient({ locale, items }: { locale: Locale; items: NewsItem[] }) {
  ...
  const filtered = useMemo(() => items.filter((item) => category === "all" || item.category === category).filter((item) => source === "all" || item.source === source).filter((item) => !savedOnly || favorites.includes(item.id)).filter((item) => `${item.title} ${item.source} ${item.sourceSummary ?? ""}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())), [category, favorites, items, query, savedOnly, source]);
  ...
  return <section className="news-feed" aria-label={t.view}>
    ...
    <div className="news-grid">{visible.map((item, index) => { const lens = insights[locale][item.category]; const terms = glossary.filter((entry) => entry.pattern.test(item.title)).map((entry) => entry.terms[locale]); return <article className={`${reviewed.includes(item.id) ? "reviewed" : ""}${index === 0 ? " featured" : ""}`} key={item.id}>
      ...
      <section className="news-quick-read"><strong>{t.quick}</strong><p>{lens.brief}</p></section>
      <section className="news-why"><strong>{t.why}</strong><p>{lens.why}</p></section>
      ...
      {item.sourceSummary && <details className="news-source-summary"><summary>{t.sourceBrief}<span>+</span></summary><p lang={item.sourceLanguage}>{item.sourceSummary}</p><small>{t.sourceNote}</small></details>}
