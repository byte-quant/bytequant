...
export function NewsFeedClient({ locale, items }: { locale: Locale; items: NewsItem[] }) {
  ...
  return <section className="news-feed" aria-label={t.view}>
    ...
    <div className="news-source-row"><span>{t.sourceFilter}</span><button type="button" aria-pressed={source === "all"} className={source === "all" ? "active" : ""} onClick={() => setSource("all")}>{t.allSources}</button>{sources.map((item) => <button type="button" aria-pressed={source === item} className={source === item ? "active" : ""} key={item} onClick={() => setSource(item)}>{item}</button>)}</div>
