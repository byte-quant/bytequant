...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: tool.title.zh, description: tool.description.zh, robots: isToolAlias(slug) ? nonIndexableRobots : undefined, alternates: localizedAlternates("zh", toolPath("tr", canonicalSlug), toolPath("en", canonicalSlug)), openGraph: { type: "website", siteName: "ByteQuant", locale: "zh_CN", alternateLocale: ["tr_TR", "en_US", "de_DE"], url: absoluteUrl(toolPath("zh", canonicalSlug)), title, description: tool.description.zh, images: [{ url: ogImageUrl, width: 1200, height: 630, alt: title }] }, twitter: { card: "summary_large_image", title, description: tool.description.zh, images: [ogImageUrl] } };
}
