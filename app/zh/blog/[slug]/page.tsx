...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const copy = guide.copy.zh; const title = `ByteQuant · ${copy.title}`; const description = copy.description.length >= 45 ? copy.description : `${copy.description} 本指南进一步说明可重复的方法、适用边界、失败条件与核验步骤。`;
  const canonicalSlug = guide.slug;
  return { title: copy.title, description, robots: slug === canonicalSlug ? undefined : { index: false, follow: true }, alternates: { canonical: absoluteUrl(postPath("zh", canonicalSlug)), languages: { "tr-TR": absoluteUrl(postPath("tr", canonicalSlug)), "en-US": absoluteUrl(postPath("en", canonicalSlug)), "de-DE": absoluteUrl(postPath("de", canonicalSlug)), "zh-CN": absoluteUrl(postPath("zh", canonicalSlug)), "x-default": absoluteUrl(postPath("en", canonicalSlug)) } }, openGraph: { type: "article", siteName: "ByteQuant", locale: "zh_CN", alternateLocale: ["tr_TR", "en_US", "de_DE"], url: absoluteUrl(postPath("zh", canonicalSlug)), title, description, publishedTime: guide.date, modifiedTime: guide.updated ?? guide.date, images: [{ url: ogImageUrl, width: 1200, height: 630, alt: title }] }, twitter: { card: "summary_large_image", title, description, images: [ogImageUrl] } };
}
