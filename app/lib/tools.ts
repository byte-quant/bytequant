...
import { frontierTools } from "./frontier-tools";
import { stageTwoTools } from "./stage-two-tools";
...
const demandTools: BaseTool[] = [
  {
    slug: "yaml-json-donusturucu", category: "data", mark: "63",
    title: { tr: "YAML ↔ JSON Dönüştürücü", en: "YAML ↔ JSON Converter" },
    short: { tr: "YAML ve JSON yapılarını güvenli ayrıştırma ve açık hata mesajlarıyla dönüştürün.", en: "Convert YAML and JSON with safe parsing and clear error messages." },
    description: { tr: "YAML belgelerini güvenli şema ile JSON'a, geçerli JSON'u okunabilir YAML'e dönüştürür. Alias genişlemesi ve belge boyutu sınırlandırılır; özel etiketler çalıştırılmaz. Sonuç yapılandırma dosyası olarak kullanılmadan önce hedef uygulamanın şemasıyla doğrulanmalıdır.", en: "Converts YAML documents to JSON with a safe schema and valid JSON to readable YAML. Alias expansion and document size are bounded, and custom tags are not executed. Validate output against the target application's schema before using it as configuration." },
    useCases: { tr: ["CI yapılandırması dönüştürme", "API örneğini YAML olarak paylaşma", "Girinti hatasını görünür kılma"], en: ["Converting CI configuration", "Sharing API samples as YAML", "Surfacing indentation errors"] },
    steps: { tr: ["YAML veya JSON metnini yapıştırıp kaynak biçimini seçin.", "Yerel dönüştürmeyi çalıştırın ve ayrıştırma uyarılarını inceleyin.", "Çıktıyı hedef şema ve uygulama davranışıyla doğrulayın."], en: ["Paste YAML or JSON and choose the source format.", "Run the local conversion and review parsing warnings.", "Validate output against the target schema and application behavior."] },
  },
  {
    slug: "xml-bicimlendirici-dogrulayici", category: "data", mark: "64",
    title: { tr: "XML Biçimlendirici ve Doğrulayıcı", en: "XML Formatter & Validator" },
    short: { tr: "XML sözdizimini doğrulayın, biçimlendirin veya küçültün.", en: "Validate, format, or minify XML syntax locally." },
    description: { tr: "XML'i tarayıcının DOMParser API'siyle ayrıştırır, parser hatalarını gösterir ve okunabilir ya da küçültülmüş çıktı üretir. XSD doğrulaması, dış varlık çözümleme ve XML dijital imza doğrulaması yapmaz.", en: "Parses XML with the browser DOMParser API, surfaces parser errors, and produces readable or minified output. It does not perform XSD validation, external entity resolution, or XML digital-signature verification." },
    useCases: { tr: ["API/XML yanıtı inceleme", "Yapılandırma dosyası düzenleme", "Kapanmayan etiketleri bulma"], en: ["Inspecting API/XML responses", "Editing configuration files", "Finding unclosed tags"] },
    steps: { tr: ["XML belgesini yapıştırın ve çıktı modunu seçin.", "Yerel ayrıştırmayı çalıştırıp hata konumunu inceleyin.", "Biçimlendirilmiş sonucu hedef XSD veya uygulamayla ayrıca doğrulayın."], en: ["Paste the XML document and select an output mode.", "Run local parsing and inspect any error location.", "Validate formatted output separately against the target XSD or application."] },
  },
  {
    slug: "json-flatten-unflatten", category: "data", mark: "65",
    title: { tr: "JSON Düzleştirici / Geri Açıcı", en: "JSON Flatten / Unflatten" },
    short: { tr: "İç içe JSON'u kayıpsız JSON Pointer yollarına düzleştirin ve geri kurun.", en: "Flatten nested JSON into lossless JSON Pointer paths and rebuild it." },
    description: { tr: "Nesne ve dizileri RFC 6901 uyumlu JSON Pointer yollarına dönüştürür veya düz yol haritasından iç içe yapıyı geri kurar. Anahtar kaçışları korunur; yinelenen ya da çakışan yollar açık hata üretir.", en: "Converts objects and arrays to RFC 6901 JSON Pointer paths or rebuilds nested data from a flat path map. Key escaping is preserved and duplicate or conflicting paths produce explicit errors." },
    useCases: { tr: ["Yapılandırma farkı hazırlama", "E-tabloya aktarılacak alanları görme", "Derin API alanlarını envanterleme"], en: ["Preparing configuration diffs", "Exposing fields for tabular export", "Inventorying deep API fields"] },
    steps: { tr: ["Geçerli iç içe JSON veya düz JSON Pointer haritası girin.", "Düzleştir ya da geri aç modunu seçip işlemi çalıştırın.", "Dizi türleri ve boş nesneleri örneklerle doğrulayın."], en: ["Enter valid nested JSON or a flat JSON Pointer map.", "Choose flatten or unflatten and run the operation.", "Verify array types and empty containers with representative samples."] },
  },
  {
    slug: "csv-tekil-satir-ayiklayici", category: "data", mark: "66",
    title: { tr: "CSV Tekilleştirici ve Yinelenen Satır Bulucu", en: "CSV Deduplicator & Duplicate Finder" },
    short: { tr: "Seçili sütunlara göre yinelenen CSV satırlarını bulun ve tekilleştirin.", en: "Find and remove duplicate CSV rows by selected columns." },
    description: { tr: "Virgül, noktalı virgül veya sekme ayracını algılar; tırnaklı alanları ayrıştırır ve sütun adı listesine göre ilk ya da son kaydı korur. Büyük/küçük harf ve çevre boşluk normalizasyonu görünür seçeneklerdir.", en: "Detects comma, semicolon, or tab delimiters, parses quoted fields, and keeps the first or last row by a selected column list. Case and surrounding-whitespace normalization are explicit options." },
    useCases: { tr: ["E-posta listesi temizleme", "Dışa aktarılan kayıtları tekilleştirme", "Birleşim öncesi veri kalite kontrolü"], en: ["Cleaning email lists", "Deduplicating exported records", "Data-quality checks before merging"] },
    steps: { tr: ["Başlıklı CSV'yi yapıştırın ve anahtar sütunları yazın.", "Eşleşme ve korunacak satır seçeneklerini belirleyin.", "Yinelenen raporunu inceleyip temiz CSV'yi indirin."], en: ["Paste CSV with a header and enter key columns.", "Choose matching rules and which row to retain.", "Review duplicate counts and download the cleaned CSV."] },
  },
  {
    slug: "url-sorgu-parametresi-analizoru", category: "data", mark: "67",
    title: { tr: "URL Sorgu Parametresi Analizörü", en: "URL Query Parameter Analyzer" },
    short: { tr: "URL bileşenlerini, yinelenen parametreleri ve takip etiketlerini görünür kılın.", en: "Inspect URL components, duplicate parameters, and tracking tags." },
    description: { tr: "URL'yi açmadan protokol, host, yol, hash ve sorgu parametrelerine ayırır; yinelenen anahtarları, boş değerleri ve yaygın UTM/takip alanlarını raporlar. İsteğe bağlı temiz URL yalnızca yerel metin dönüşümüdür.", en: "Splits a URL into protocol, host, path, fragment, and query parameters without opening it, and reports duplicate keys, empty values, and common UTM/tracking fields. Optional cleanup is a local text transformation only." },
    useCases: { tr: ["Kampanya URL'si hata ayıklama", "Yinelenen query alanlarını bulma", "Paylaşım öncesi takip parametresi temizleme"], en: ["Debugging campaign URLs", "Finding duplicate query fields", "Removing tracking parameters before sharing"] },
    steps: { tr: ["Tam URL'yi protokolle birlikte yapıştırın.", "Bileşen ve parametre analizini çalıştırın.", "Temizlenen adresi kullanmadan önce hedef host ve gerekli parametreleri doğrulayın."], en: ["Paste the full URL including its protocol.", "Run component and parameter analysis.", "Verify the destination host and required parameters before using the cleaned URL."] },
  },
  {
    slug: "html-varlik-kodlayici", category: "data", mark: "68",
    title: { tr: "HTML Varlık Kodlayıcı / Çözücü", en: "HTML Entity Encoder / Decoder" },
    short: { tr: "HTML özel karakterlerini entity biçimine kodlayın veya güvenli metne çözün.", en: "Encode HTML special characters as entities or decode them to text." },
    description: { tr: "Ampersand, küçüktür, büyüktür, tırnak ve Unicode sayısal varlıklarını tarayıcı içinde dönüştürür. Çözme işlemi metin çıktısı verir; HTML çalıştırmaz. Kodlama tek başına bağlama duyarlı XSS koruması değildir.", en: "Transforms ampersands, angle brackets, quotes, and numeric Unicode entities in-browser. Decoding returns text and never executes HTML. Encoding alone is not context-aware XSS protection." },
    useCases: { tr: ["Kod örneğini HTML içinde gösterme", "Entity içeren içeriği okuma", "Şablon kaçışını ön kontrol"], en: ["Displaying code inside HTML", "Reading entity-encoded content", "Pre-checking template escaping"] },
    steps: { tr: ["Metni girip kodlama veya çözme modunu seçin.", "Dönüşümü çalıştırın ve karakter sayısını karşılaştırın.", "Üretim şablonunda bağlama uygun otomatik escaping kullanın."], en: ["Enter text and choose encode or decode.", "Run the conversion and compare character counts.", "Use context-appropriate automatic escaping in production templates."] },
  },
  {
    slug: "ip-cidr-alt-ag-hesaplayici", category: "calculation", mark: "69",
    title: { tr: "IPv4 CIDR ve Alt Ağ Hesaplayıcı", en: "IPv4 CIDR & Subnet Calculator" },
    short: { tr: "IPv4/CIDR için ağ, broadcast, host aralığı ve kullanılabilir adres sayısını hesaplayın.", en: "Calculate network, broadcast, host range, and usable addresses for IPv4/CIDR." },
    description: { tr: "IPv4 adresi ve /0–/32 önekini 32 bit tamsayı aritmetiğiyle hesaplar; netmask, wildcard, ağ, broadcast ve klasik kullanılabilir host aralığını gösterir. Yönlendirici politikası, IPv6 ve özel bulut rezervasyonları dahil değildir.", en: "Uses 32-bit integer arithmetic for an IPv4 address and /0–/32 prefix and reports netmask, wildcard, network, broadcast, and the traditional usable-host range. Router policy, IPv6, and cloud-specific reservations are excluded." },
    useCases: { tr: ["VLAN planlama ön hesabı", "CIDR sınırı doğrulama", "Firewall kuralı inceleme"], en: ["VLAN planning estimates", "Validating CIDR boundaries", "Reviewing firewall ranges"] },
    steps: { tr: ["IPv4 adresini CIDR önekiyle girin.", "Alt ağ hesabını çalıştırıp ikili sınırları inceleyin.", "Sonucu gerçek ağ, bulut sağlayıcı ve yönlendirme politikasıyla doğrulayın."], en: ["Enter an IPv4 address with its CIDR prefix.", "Run subnet calculation and inspect binary boundaries.", "Verify output against the real network, cloud provider, and routing policy."] },
  },
  {
    slug: "robots-txt-olusturucu-denetleyici", category: "data", mark: "70",
    title: { tr: "Robots.txt Oluşturucu ve Denetleyici", en: "Robots.txt Builder & Auditor" },
    short: { tr: "Robots.txt kurallarını ayrıştırın, çakışmaları ve kritik engelleri bulun.", en: "Parse robots.txt rules and find conflicts or critical crawl blocks." },
    description: { tr: "User-agent grupları, Allow, Disallow, Sitemap ve Crawl-delay satırlarını yerel olarak denetler; kök engeli, göreli sitemap ve bilinmeyen direktifleri işaretler. Gerçek crawler davranışı ve indeksleme sonucu garanti edilmez.", en: "Audits user-agent groups, Allow, Disallow, Sitemap, and Crawl-delay lines locally and flags root blocks, relative sitemaps, and unknown directives. Actual crawler behavior and indexing outcomes are not guaranteed." },
    useCases: { tr: ["Yayın öncesi crawl kontrolü", "AI bot kurallarını envanterleme", "Sitemap bildirimini doğrulama"], en: ["Pre-launch crawl checks", "Inventorying AI-bot rules", "Validating sitemap declarations"] },
    steps: { tr: ["Mevcut robots.txt metnini yapıştırın veya örnek yükleyin.", "Yapısal denetimi çalıştırıp agent gruplarını inceleyin.", "Canlı alan adında Search Console URL Denetimi ile gerçek erişimi doğrulayın."], en: ["Paste the current robots.txt or load the example.", "Run structural auditing and inspect agent groups.", "Verify real access on the live domain with Search Console URL Inspection."] },
  },
  {
    slug: "hreflang-etiket-olusturucu", category: "data", mark: "71",
    title: { tr: "Hreflang Etiket Oluşturucu ve Denetleyici", en: "Hreflang Tag Generator & Auditor" },
    short: { tr: "Dil-bölge URL eşleşmelerinden karşılıklı hreflang etiketi ve sitemap satırları üretin.", en: "Generate reciprocal hreflang tags and sitemap entries from locale URL mappings." },
    description: { tr: "BCP 47 dil/bölge kodlarını ve mutlak HTTPS URL'leri doğrular; kendine referans, yinelenen dil ve x-default eksiklerini gösterir. HTML link etiketleri ile XML sitemap xhtml:link taslağı üretir.", en: "Validates BCP 47 language/region codes and absolute HTTPS URLs and reports missing self-reference, duplicate languages, or x-default. It generates HTML link tags and an XML sitemap xhtml:link draft." },
    useCases: { tr: ["Çok dilli sayfa yayını", "Hreflang kümesi kalite kontrolü", "Sitemap alternates hazırlama"], en: ["Publishing multilingual pages", "QA for hreflang clusters", "Preparing sitemap alternates"] },
    steps: { tr: ["Her satıra `dil|mutlak URL` eşleşmesi girin.", "Etiketleri üretip kod ve URL uyarılarını düzeltin.", "Her dil sayfasında aynı karşılıklı kümenin bulunduğunu doğrulayın."], en: ["Enter one `language|absolute URL` mapping per line.", "Generate tags and fix code or URL warnings.", "Verify the identical reciprocal cluster appears on every language page."] },
  },
  {
    slug: "faq-json-ld-olusturucu", category: "data", mark: "72",
    title: { tr: "FAQPage JSON-LD Oluşturucu", en: "FAQPage JSON-LD Generator" },
    short: { tr: "Görünür soru-cevaplardan geçerli FAQPage JSON-LD taslağı üretin.", en: "Build valid FAQPage JSON-LD from visible questions and answers." },
    description: { tr: "`Soru | Yanıt` satırlarını Schema.org FAQPage yapısına dönüştürür, boş ve yinelenen soruları denetler. İşaretlenen içerik sayfada kullanıcıya görünür olmalıdır; şema zengin sonuç veya sıralama garantisi vermez.", en: "Converts `Question | Answer` rows into Schema.org FAQPage, checking empty and duplicate questions. Marked-up content must also be visible to users; schema does not guarantee rich results or ranking." },
    useCases: { tr: ["SSS sayfası JSON-LD hazırlama", "Soru-cevap tutarlılığı denetimi", "Schema taslağını dışa aktarma"], en: ["Preparing FAQ JSON-LD", "Auditing question-answer consistency", "Exporting a schema draft"] },
    steps: { tr: ["Her görünür soru ve yanıtı `Soru | Yanıt` biçiminde girin.", "JSON-LD üretip yinelenen veya kısa içerik uyarılarını inceleyin.", "Şemadaki tüm metnin aynı canonical sayfada görünür olduğunu doğrulayın."], en: ["Enter each visible question and answer as `Question | Answer`.", "Generate JSON-LD and review duplicate or thin-content warnings.", "Verify every marked-up statement is visible on the same canonical page."] },
  },
  {
    slug: "utm-kampanya-url-olusturucu", category: "data", mark: "73",
    title: { tr: "UTM Kampanya URL Oluşturucu", en: "UTM Campaign URL Builder" },
    short: { tr: "Kaynak, mecra, kampanya ve içerik etiketleriyle tutarlı izleme URL'si oluşturun.", en: "Build consistent tracking URLs with source, medium, campaign, and content tags." },
    description: { tr: "Geçerli HTTP(S) hedef URL'sine UTM parametrelerini güvenli URL kodlamasıyla ekler, mevcut query/hash değerlerini korur ve adlandırma tutarsızlıklarını uyarır. Araç URL'yi açmaz veya analitik servisine göndermez.", en: "Adds UTM parameters to a valid HTTP(S) destination with safe URL encoding, preserves existing query and fragment values, and warns about inconsistent naming. It never opens the URL or contacts analytics services." },
    useCases: { tr: ["Bülten bağlantısı hazırlama", "Sosyal kampanya adlandırma", "UTM standardını ekipte koruma"], en: ["Preparing newsletter links", "Naming social campaigns", "Enforcing a team UTM standard"] },
    steps: { tr: ["Hedef URL'yi ve kampanya alanlarını girin.", "Kodlanmış bağlantıyı üretip host ve parametreleri inceleyin.", "Gerçek kampanya başlamadan önce test tıklamasını analitik sisteminizde doğrulayın."], en: ["Enter the destination URL and campaign fields.", "Generate the encoded link and inspect host and parameters.", "Before launch, verify a test click in your analytics system."] },
  },
  {
    slug: "unicode-normalizasyon-inceleyici", category: "text", mark: "74",
    title: { tr: "Unicode Normalizasyonu ve Karakter İnceleyici", en: "Unicode Normalizer & Character Inspector" },
    short: { tr: "NFC/NFD/NFKC/NFKD biçimlerini, kod noktalarını ve görünmez karakterleri inceleyin.", en: "Inspect NFC/NFD/NFKC/NFKD forms, code points, and invisible characters." },
    description: { tr: "Metni seçilen Unicode normalizasyon biçimine dönüştürür; her kod noktası, UTF-16 uzunluğu, birleşen işaret ve yaygın görünmez/bidi karakterlerini raporlar. Normalizasyon görsel benzerlik veya kimlik güvenliği sağlamaz.", en: "Normalizes text to a selected Unicode form and reports code points, UTF-16 length, combining marks, and common invisible or bidi characters. Normalization does not establish visual equivalence or identity safety." },
    useCases: { tr: ["Kopyalama kaynaklı karakter farkı", "Kimlik/slug ön temizliği", "Görünmez yön karakteri bulma"], en: ["Diagnosing copy/paste character differences", "Pre-cleaning identifiers or slugs", "Finding invisible direction controls"] },
    steps: { tr: ["İncelenecek metni girip normalizasyon biçimini seçin.", "Dönüşümü çalıştırıp kod noktası ve görünmez karakter raporunu okuyun.", "Kimlik ve güvenlik kontrollerinde confusable karakterleri ayrıca inceleyin."], en: ["Enter text and select a normalization form.", "Run processing and review code points and invisible-character findings.", "For identity or security checks, inspect confusable characters separately."] },
  },
  {
    slug: "satir-siralayici-tekillestirici", category: "text", mark: "75",
    title: { tr: "Satır Sıralayıcı ve Tekilleştirici", en: "Line Sorter & Deduplicator" },
    short: { tr: "Satırları dile duyarlı sıralayın, boşları ve yinelenenleri kontrollü kaldırın.", en: "Sort lines with locale awareness and remove blanks or duplicates explicitly." },
    description: { tr: "Satırları Intl.Collator ile alfabetik, doğal sayısal veya ters sıraya dizer; trim ve büyük/küçük harf duyarlılığını seçenek olarak uygular. İlk görülen özgün satırı koruyarak yinelenen raporu üretir.", en: "Sorts lines with Intl.Collator using alphabetical, natural numeric, or reverse order, with explicit trim and case-sensitivity options. It preserves the first unique line and reports duplicates." },
    useCases: { tr: ["Anahtar/değer listesi düzenleme", "İçerik envanteri temizleme", "Test verisini kararlı sıralama"], en: ["Organizing key/value lists", "Cleaning content inventories", "Stable sorting of test data"] },
    steps: { tr: ["Her öğeyi ayrı satıra yapıştırın.", "Sıralama ve tekilleştirme kurallarını seçin.", "Çıktı sayılarıyla veri kaybı olup olmadığını kontrol edin."], en: ["Paste one item per line.", "Choose sorting and deduplication rules.", "Use the counts to verify that no required data was removed."] },
  },
  {
    slug: "seo-slug-olusturucu", category: "text", mark: "76",
    title: { tr: "SEO ve URL Slug Oluşturucu", en: "SEO & URL Slug Generator" },
    short: { tr: "Başlıkları kararlı, küçük harfli ve URL güvenli slug biçimine dönüştürün.", en: "Turn titles into stable lowercase URL-safe slugs." },
    description: { tr: "Unicode metni normalize eder; Türkçe ve Almanca karakterleri açık eşlemeyle, diğer Latin işaretleri NFKD ile ASCII slug'a çevirir. ASCII karşılığı olmayan karakterler kaybolmak yerine `u-kod` biçiminde korunur. Mevcut yayın URL'leri otomatik değiştirilmemelidir.", en: "Normalizes Unicode text, maps Turkish and German characters explicitly, and converts other Latin marks with NFKD. Characters without an ASCII mapping are retained as `u-code` tokens rather than silently dropped. Existing published URLs should not be changed automatically." },
    useCases: { tr: ["Yeni makale URL'si hazırlama", "Dosya adını standartlaştırma", "Çok dilli slug çakışmasını bulma"], en: ["Preparing new article URLs", "Standardizing filenames", "Finding multilingual slug collisions"] },
    steps: { tr: ["Başlığı veya ad listesini girin.", "Slug üretip boş, çok uzun veya çakışan sonuçları inceleyin.", "Yayınlanmış URL değişecekse kalıcı yönlendirme ve canonical planlayın."], en: ["Enter a title or list of names.", "Generate slugs and inspect empty, long, or duplicate results.", "If changing a published URL, plan a permanent redirect and canonical update."] },
  },
  {
    slug: "kelime-sikligi-ngram-analizi", category: "text", mark: "77",
    title: { tr: "Kelime Sıklığı ve N-gram Analizi", en: "Word Frequency & N-gram Analyzer" },
    short: { tr: "Metindeki en sık sözcük, ikili ve üçlü dizileri yerel olarak bulun.", en: "Find frequent words, bigrams, and trigrams locally." },
    description: { tr: "Intl.Segmenter ile dile duyarlı sözcük parçalama yapar, 1–3 kelimelik n-gramları sayar ve oranları gösterir. Sonuç konu, anlam, kalite veya anahtar kelime sıralama garantisi değildir.", en: "Uses Intl.Segmenter for locale-aware word segmentation, counts one- to three-word n-grams, and reports proportions. Results do not establish topic, meaning, quality, or keyword ranking." },
    useCases: { tr: ["İçerik tekrarlarını bulma", "Müşteri geri bildirimini ön inceleme", "Terminoloji tutarlılığı kontrolü"], en: ["Finding content repetition", "First-pass feedback analysis", "Checking terminology consistency"] },
    steps: { tr: ["Temsil gücü yüksek metni yapıştırın ve n-gram boyutunu seçin.", "Sıklık analizini çalıştırıp örneklem boyutunu inceleyin.", "Sayıları bağlam ve gerçek kullanıcı amacıyla birlikte yorumlayın."], en: ["Paste representative text and choose n-gram size.", "Run frequency analysis and inspect sample size.", "Interpret counts together with context and real user intent."] },
  },
  {
    slug: "yuzde-degisim-hesaplayici", category: "calculation", mark: "78",
    title: { tr: "Yüzde Değişim, Artış ve Pay Hesaplayıcı", en: "Percentage Change, Increase & Share Calculator" },
    short: { tr: "Eski-yeni değer, parçanın bütüne oranı ve hedef artışı tek raporda hesaplayın.", en: "Calculate old-to-new change, part-to-whole share, and target increase in one report." },
    description: { tr: "Yüzde değişim, yüzde puan farkı, bir değerin bütündeki payı ve belirli yüzdeyle artırılmış/azaltılmış sonucu açık formüllerle hesaplar. Sıfır veya negatif tabanlarda matematiksel sınırları görünür biçimde uyarır.", en: "Calculates percentage change, percentage-point difference, part-to-whole share, and values increased or decreased by a percentage using disclosed formulas. It explicitly warns about zero or negative baselines." },
    useCases: { tr: ["Aylık performans karşılaştırma", "İndirim/zam ön hesabı", "Oran ve yüzde puanını ayırma"], en: ["Comparing monthly performance", "Estimating discounts or increases", "Separating percent from percentage points"] },
    steps: { tr: ["Eski, yeni, parça, bütün ve oran değerlerinden gerekenleri girin.", "Hesabı çalıştırıp kullanılan formülleri inceleyin.", "Yuvarlama ve negatif taban yorumunu rapor bağlamına göre doğrulayın."], en: ["Enter the old, new, part, whole, or rate values you need.", "Run calculation and review the disclosed formulas.", "Validate rounding and negative-baseline interpretation for your reporting context."] },
  },
  {
    slug: "kdv-indirim-hesaplayici", category: "calculation", mark: "79",
    title: { tr: "KDV / Vergi ve İndirim Hesaplayıcı", en: "VAT / Tax & Discount Calculator" },
    short: { tr: "Vergi dahil-hariç fiyatı ve ardışık indirimi şeffaf adımlarla hesaplayın.", en: "Calculate tax-inclusive/exclusive prices and sequential discounts transparently." },
    description: { tr: "Kullanıcının girdiği vergi oranıyla dahil veya hariç fiyattan matrah/vergi çıkarır ve birden fazla indirimi sırayla uygular. Oran seçmez, mevzuat doğrulamaz ve fatura ya da vergi tavsiyesi üretmez.", en: "Uses a user-supplied tax rate to derive net and tax from inclusive or exclusive prices and applies multiple discounts sequentially. It does not choose rates, verify law, or provide invoicing or tax advice." },
    useCases: { tr: ["Fiyat etiketi ön hesabı", "Ardışık kampanya indirimi", "Vergi dahil-hariç karşılaştırma"], en: ["Price-label estimates", "Sequential campaign discounts", "Comparing tax-inclusive and exclusive values"] },
    steps: { tr: ["Fiyat, vergi oranı, dahil/hariç modu ve indirimleri girin.", "Hesaplamayı çalıştırıp her ara adımı kontrol edin.", "Geçerli oran, yuvarlama ve belge kurallarını resmî kaynaktan doğrulayın."], en: ["Enter price, tax rate, inclusive/exclusive mode, and discounts.", "Run the calculation and inspect every intermediate step.", "Verify current rates, rounding, and document rules from an official source."] },
  },
  {
    slug: "sure-mesai-hesaplayici", category: "calculation", mark: "80",
    title: { tr: "Süre, Mesai ve Mola Hesaplayıcı", en: "Duration, Worktime & Break Calculator" },
    short: { tr: "Başlangıç-bitiş, gece aşımı ve mola ile net süreyi hesaplayın.", en: "Calculate net duration with start/end times, overnight spans, and breaks." },
    description: { tr: "İki tarih-saat arasındaki gerçek milisaniye farkını hesaplar, mola dakikalarını düşer ve gün/saat/dakika olarak gösterir. Saat dilimi ofseti girdide yoksa tarayıcının yerel saat dilimi kullanılır; bordro ve hukuki mesai kuralı uygulanmaz.", en: "Calculates elapsed milliseconds between two date-times, subtracts break minutes, and reports days, hours, and minutes. If inputs omit an offset, the browser's local time zone is used; payroll and legal worktime rules are not applied." },
    useCases: { tr: ["Vardiya süresi ön hesabı", "Proje çalışma kaydı", "Gece aşan etkinlik planı"], en: ["Shift-duration estimates", "Project time records", "Planning overnight events"] },
    steps: { tr: ["Başlangıç, bitiş ve toplam mola dakikasını girin.", "Net ve brüt süreyi hesaplayıp saat dilimi bilgisini inceleyin.", "Bordro, yuvarlama ve yasal dinlenme kurallarını ayrı uygulayın."], en: ["Enter start, end, and total break minutes.", "Calculate gross and net duration and inspect time-zone assumptions.", "Apply payroll, rounding, and statutory-rest rules separately."] },
  },
  {
    slug: "rastgele-secici-takim-karistirici", category: "general", mark: "81",
    title: { tr: "Kriptografik Rastgele Seçici ve Takım Karıştırıcı", en: "Cryptographic Random Picker & Team Shuffler" },
    short: { tr: "Listeyi Web Crypto ile tarafsız karıştırın, seçim yapın veya takımlara bölün.", en: "Shuffle a list with Web Crypto, pick entries, or split teams." },
    description: { tr: "Her satırdaki öğeyi kriptografik rastgele Fisher–Yates algoritmasıyla karıştırır; istenen sayıda kazanan veya dengeli takım üretir. Girdi kalitesi, katılım hakkı ve çekiliş mevzuatı araç tarafından doğrulanmaz.", en: "Uses a cryptographically random Fisher–Yates shuffle for one item per line and produces a requested number of winners or balanced teams. It does not verify eligibility, input fairness, or sweepstakes law." },
    useCases: { tr: ["Toplantı konuşma sırası", "Atölye grubu oluşturma", "Düşük riskli rastgele seçim"], en: ["Meeting speaking order", "Workshop team creation", "Low-stakes random selection"] },
    steps: { tr: ["Her benzersiz katılımcıyı ayrı satıra girin.", "Seçim veya takım modunu ve sayıyı belirleyin.", "Sonucu kaydedin; düzenlemeye tabi çekilişlerde resmî süreci kullanın."], en: ["Enter each unique participant on a separate line.", "Choose picker or team mode and a count.", "Record the result and use an official process for regulated drawings."] },
  },
  {
    slug: "hatirlanabilir-parola-uretici", category: "security", mark: "82",
    title: { tr: "Hatırlanabilir Parola / Passphrase Üretici", en: "Memorable Passphrase Generator" },
    short: { tr: "Web Crypto ile okunabilir sentetik sözcüklerden uzun passphrase üretin.", en: "Generate long passphrases from readable synthetic words with Web Crypto." },
    description: { tr: "Harici kelime listesi kullanmadan kriptografik rastgele, telaffuz edilebilir sentetik parçalar oluşturur; sözcük sayısı, ayraç ve isteğe bağlı rakam seçeneği sunar. Entropi teorik üst sınırdır; parola yöneticisi ve benzersiz kullanım önerilir.", en: "Creates cryptographically random, pronounceable synthetic tokens without an external word list, with controls for word count, separator, and optional digits. Reported entropy is a theoretical upper bound; unique use and a password manager are recommended." },
    useCases: { tr: ["Uzun ana parola taslağı", "Wi-Fi parolası", "Okunabilir geçici sır"], en: ["Long master-password draft", "Wi-Fi passphrase", "Readable temporary secret"] },
    steps: { tr: ["Sözcük sayısı ve ayırıcı seçeneklerini belirleyin.", "Web Crypto ile passphrase üretip entropi tahminini inceleyin.", "Sonucu benzersiz kullanın ve güvenilir parola yöneticisinde saklayın."], en: ["Choose word count and separator options.", "Generate with Web Crypto and inspect the entropy estimate.", "Use it uniquely and store it in a trusted password manager."] },
  },
  {
    slug: "hmac-olusturucu-dogrulayici", category: "security", mark: "83",
    title: { tr: "HMAC Oluşturucu ve Doğrulayıcı", en: "HMAC Generator & Verifier" },
    short: { tr: "HMAC-SHA-256/384/512 üretin ve isteğe bağlı beklenen özeti yerelde karşılaştırın.", en: "Generate HMAC-SHA-256/384/512 and locally compare an optional expected digest." },
    description: { tr: "Web Crypto ile UTF-8 anahtar ve mesajdan HMAC üretir; hex veya Base64URL çıktısı verir ve isteğe bağlı beklenen değeri uzunluk-normalize edilmiş bayt karşılaştırmasıyla denetler. JavaScript çalışma zamanı sabit zaman garantisi vermez. Anahtar paylaşılmamalı; HMAC şifreleme, kimlik belgesi veya açık anahtarlı imza değildir.", en: "Uses Web Crypto to create HMAC from a UTF-8 key and message, outputs hex or Base64URL, and checks an optional expected value with a length-normalized byte comparison. JavaScript runtimes do not guarantee constant-time execution. Keys must remain secret; HMAC is not encryption, identity proof, or a public-key signature." },
    useCases: { tr: ["Webhook imzası hata ayıklama", "Test fixture üretme", "Mesaj bütünlüğü doğrulama"], en: ["Debugging webhook signatures", "Creating test fixtures", "Verifying message integrity"] },
    steps: { tr: ["Yalnızca test veya kullanmaya yetkili olduğunuz anahtar ve mesajı girin.", "Algoritma ve çıktı biçimini seçip HMAC üretin.", "Üretim doğrulamasında sağlayıcının byte kodlama ve canonicalization kuralını aynen uygulayın."], en: ["Enter only a test key and message you are authorized to use.", "Choose algorithm and output format, then generate the HMAC.", "In production, follow the provider's byte encoding and canonicalization rules exactly."] },
  },
  {
    slug: "sri-butunluk-hash-uretici", category: "security", mark: "84",
    title: { tr: "Subresource Integrity (SRI) Hash Üretici", en: "Subresource Integrity (SRI) Hash Generator" },
    short: { tr: "Yerel dosya veya metin için sha256/384/512 SRI integrity değeri üretin.", en: "Generate sha256/384/512 SRI integrity values for a local file or text." },
    description: { tr: "Web Crypto ile seçilen dosyanın tam baytlarından veya yapıştırılan UTF-8 metinden standart `sha384-base64` integrity değeri üretir. SRI kaynağın sonradan değişmesini yakalar; ilk kaynağın güvenli veya sahibinin doğru olduğunu kanıtlamaz.", en: "Uses Web Crypto to generate a standard `sha384-base64` integrity value from exact local-file bytes or pasted UTF-8 text. SRI detects later resource changes; it does not prove the original resource is safe or correctly owned." },
    useCases: { tr: ["CDN script bütünlüğü sabitleme", "Stil dosyası hash'i üretme", "Yayın artefaktı değişimini izleme"], en: ["Pinning CDN script integrity", "Hashing stylesheets", "Tracking release-artifact changes"] },
    steps: { tr: ["Yerel dosyayı seçin veya tam kaynak metnini yapıştırın.", "Algoritmayı seçip SRI değerini cihazınızda üretin.", "HTML etiketi, CORS davranışı ve güncelleme sürecini gerçek ortamda test edin."], en: ["Choose a local file or paste the exact resource text.", "Select an algorithm and generate SRI on-device.", "Test the HTML tag, CORS behavior, and update process in the real environment."] },
  },
  {
    slug: "rag-parcalama-butcesi-planlayici", category: "ai", mark: "85",
    title: { tr: "RAG Parçalama ve Bağlam Bütçesi Planlayıcı", en: "RAG Chunking & Context Budget Planner" },
    short: { tr: "Belge tokenı, chunk, overlap, sonuç sayısı ve prompt rezervini birlikte planlayın.", en: "Plan document tokens, chunks, overlap, result count, and prompt reserve together." },
    description: { tr: "Belge büyüklüğü, hedef chunk ve overlap değerlerinden yaklaşık parça sayısı; getirilecek parça sayısı ve bağlam penceresinden kalan çıktı bütçesini hesaplar. Retrieval kalitesi, tokenizer, embedding modeli ve belge yapısı için deneysel doğrulama gerekir.", en: "Estimates chunk count from document size, target chunk, and overlap, then calculates retrieved context and remaining output budget for a context window. Retrieval quality, tokenizer, embedding model, and document structure require empirical validation." },
    useCases: { tr: ["RAG prototipi kapasite planı", "Chunk/overlap senaryosu karşılaştırma", "Bağlam taşması ön kontrolü"], en: ["Capacity planning for RAG prototypes", "Comparing chunk/overlap scenarios", "Pre-checking context overflow"] },
    steps: { tr: ["Belge, bağlam, chunk, overlap ve getirilecek parça sayılarını girin.", "Planı çalıştırıp tekrar yükü ve çıktı rezervini inceleyin.", "Gerçek tokenizer, retrieval ölçütleri ve temsil gücü yüksek sorularla değerlendirin."], en: ["Enter document, context, chunk, overlap, and retrieved-chunk values.", "Run planning and inspect duplication load and output reserve.", "Evaluate with the real tokenizer, retrieval metrics, and representative questions."] },
  },
  {
    slug: "prompt-enjeksiyon-on-taramasi", category: "ai", mark: "86",
    title: { tr: "Prompt Enjeksiyonu Risk Ön Taraması", en: "Prompt Injection Risk Pre-Scan" },
    short: { tr: "Metindeki talimat ezme, sır isteme ve araç kötüye kullanım sinyallerini yerel kurallarla bulun.", en: "Find instruction override, secret requests, and tool-abuse signals with local rules." },
    description: { tr: "Kullanıcı, web veya RAG metnini zaman sınırlı açıklanabilir kurallarla tarar; rol değiştirme, önceki talimatı yok sayma, sır/sistem promptu isteme ve kodlanmış komut sinyallerini satır bazında raporlar. Bu semantik sınıflandırma, sandbox veya jailbreak garantisi değildir.", en: "Scans user, web, or RAG text with time-bounded explainable rules and reports role changes, ignore-previous-instruction phrases, secret or system-prompt requests, and encoded-command signals by line. It is not semantic classification, sandboxing, or a jailbreak guarantee." },
    useCases: { tr: ["RAG içeriği ilk triyaj", "Araç kullanan ajan girdisi kontrolü", "Red-team senaryosu hazırlama"], en: ["First-pass RAG-content triage", "Checking tool-using agent input", "Preparing red-team scenarios"] },
    steps: { tr: ["Yalnızca incelemeye yetkili olduğunuz metni yapıştırın.", "Yerel ön taramayı çalıştırıp her bulgunun satır ve kuralını inceleyin.", "Kaynak güveni, ayrı talimat/veri kanalı, allowlist ve gerçek model testleriyle doğrulayın."], en: ["Paste only text you are authorized to inspect.", "Run the local pre-scan and review each finding's line and rule.", "Verify with source trust, separate instruction/data channels, allowlists, and real model tests."] },
  },
  {
    slug: "prompt-test-vaka-matrisi", category: "prompt", mark: "87",
    title: { tr: "Prompt Test Vaka Matrisi Oluşturucu", en: "Prompt Test Case Matrix Builder" },
    short: { tr: "Bir prompt için normal, sınır, kötüye kullanım ve belirsizlik test vakaları üretin.", en: "Build normal, boundary, abuse, and ambiguity test cases for a prompt." },
    description: { tr: "Görev, hedef davranış ve risk alanlarından tekrarlanabilir bir test matrisi taslağı üretir; başarı ölçütü, beklenen davranış ve insan inceleme notunu ayrı sütunlarda gösterir. Model çağırmaz, testleri çalıştırmaz ve güvenlik sertifikası vermez.", en: "Builds a repeatable test-matrix draft from a task, desired behavior, and risk areas, separating success criteria, expected behavior, and human-review notes. It does not call a model, execute tests, or provide security certification." },
    useCases: { tr: ["Prompt regresyon seti hazırlama", "Sınır ve kötüye kullanım vakalarını görünür kılma", "İnsan değerlendirme planı oluşturma"], en: ["Preparing prompt regression sets", "Surfacing boundary and abuse cases", "Planning human evaluation"] },
    steps: { tr: ["Görevi ve satır başına hedef davranış veya risk alanını girin.", "Yerel matrisi üretip her vakanın beklenen davranışını düzenleyin.", "Vakaları gerçek model/sürüm üzerinde çalıştırıp gözlenen sonucu ve değerlendiriciyi kaydedin."], en: ["Enter the task and one desired behavior or risk area per line.", "Generate the local matrix and edit each expected behavior.", "Run cases against the real model/version and record observed outcomes and reviewers."] },
  },
  {
    slug: "data-uri-donusturucu", category: "converter", mark: "88",
    title: { tr: "Data URI Kodlayıcı / Çözücü", en: "Data URI Encoder / Decoder" },
    short: { tr: "Metin veya küçük dosyayı Data URI'ye dönüştürün; Data URI meta verisini ve içeriğini çözün.", en: "Convert text or a small file to a Data URI and decode Data URI metadata and content." },
    description: { tr: "Tarayıcı FileReader ve Base64 API'leriyle en fazla 5 MB dosya veya UTF-8 metin için Data URI üretir; geçerli Data URI'nin MIME türü, kodlama biçimi ve byte içeriğini yerelde çözer. Data URI güvenilirlik, zararsızlık veya MIME doğruluğu kanıtı değildir.", en: "Uses browser FileReader and Base64 APIs to create a Data URI from a file up to 5 MB or UTF-8 text, and locally decodes a valid Data URI's media type, encoding, and bytes. A Data URI does not prove trust, safety, or MIME accuracy." },
    useCases: { tr: ["Küçük SVG/CSS varlığını gömme", "Data URI hata ayıklama", "UTF-8 metni taşınabilir örneğe dönüştürme"], en: ["Embedding a small SVG/CSS asset", "Debugging Data URIs", "Turning UTF-8 text into a portable sample"] },
    steps: { tr: ["Kodlama için metin/dosya seçin veya çözmek için Data URI yapıştırın.", "5 MB sınırı içinde yerel dönüşümü çalıştırıp MIME ve byte sayısını inceleyin.", "Çözülen içeriği çalıştırmadan önce türünü ve kaynağını bağımsız doğrulayın."], en: ["Choose text/file for encoding or paste a Data URI for decoding.", "Run local conversion within the 5 MB limit and inspect media type and byte count.", "Independently verify decoded content and origin before executing or embedding it."] },
  },
  {
    slug: "http-guvenlik-basliklari-denetleyici", category: "codeSecurity", mark: "89",
    title: { tr: "HTTP Güvenlik Başlıkları Denetleyici", en: "HTTP Security Headers Auditor" },
    short: { tr: "Yapıştırılan yanıt başlıklarında CSP, HSTS, nosniff, referrer ve izin politikalarını denetleyin.", en: "Audit pasted response headers for CSP, HSTS, nosniff, referrer, and permissions policies." },
    description: { tr: "Ham HTTP yanıt başlıklarını yerel olarak ayrıştırır; yinelenen alanları ve CSP, Strict-Transport-Security, X-Content-Type-Options, Referrer-Policy, Permissions-Policy ile çerçeveleme korumasındaki eksikleri raporlar. Sunucuya istek göndermez; TLS, uygulama kodu veya canlı yönlendirme davranışını doğrulamaz.", en: "Parses raw HTTP response headers locally and reports duplicates or gaps in CSP, Strict-Transport-Security, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, and framing protection. It sends no request and does not validate TLS, application code, or live redirect behavior." },
    useCases: { tr: ["Yayın öncesi header kontrolü", "CDN/proxy yapılandırma karşılaştırması", "CSP geçiş planı"], en: ["Pre-release header checks", "Comparing CDN/proxy configurations", "Planning a CSP rollout"] },
    steps: { tr: ["Yetkili olduğunuz bir yanıttan ham başlıkları yapıştırın.", "Yerel denetimi çalıştırıp eksik, yinelenen ve riskli değerleri inceleyin.", "Bulguları canlı HTTPS yanıtı, tarayıcı konsolu ve uygulama tehdit modeliyle doğrulayın."], en: ["Paste raw headers from a response you are authorized to inspect.", "Run the local audit and review missing, duplicate, or risky values.", "Verify findings against the live HTTPS response, browser console, and application threat model."] },
  },
];

const discoveryTools: BaseTool[] = [
  { slug: "json-lines-donusturucu", category: "data", mark: "90", title: { tr: "JSON Lines Dönüştürücü", en: "JSON Lines Converter" }, short: { tr: "JSON dizileri ile satır başına bir kayıt içeren NDJSON arasında güvenli dönüşüm yapın.", en: "Convert safely between JSON arrays and one-record-per-line NDJSON." }, description: { tr: "Büyük veri akışlarında kullanılan JSON Lines (NDJSON) biçimini satır satır doğrular; JSON dizisini NDJSON'a veya NDJSON'u tek bir JSON dizisine dönüştürür. Hatalı satırı numarasıyla gösterir ve veri hiçbir yere yüklenmez.", en: "Validates JSON Lines (NDJSON) record by record and converts a JSON array to NDJSON or NDJSON back to one JSON array. Invalid records are reported by line number and nothing is uploaded." }, useCases: { tr: ["Log dışa aktarımlarını hazırlama", "Veri kümelerini satır bazında doğrulama", "JSON API örneklerini NDJSON'a çevirme"], en: ["Preparing log exports", "Validating datasets record by record", "Converting JSON API samples to NDJSON"] }, steps: { tr: ["JSON dizisi veya NDJSON metnini yapıştırın.", "Otomatik algılamayı çalıştırın ve satır uyarılarını inceleyin.", "Doğrulanmış çıktıyı kopyalayın veya sonraki araca aktarın."], en: ["Paste a JSON array or NDJSON text.", "Run auto-detection and review any line warning.", "Copy the validated output or pass it to the next tool."] } },
  { slug: "json-pointer-gezgini", category: "data", mark: "91", title: { tr: "JSON Pointer Gezgini", en: "JSON Pointer Explorer" }, short: { tr: "RFC 6901 yollarıyla iç içe JSON değerlerini bulun ve yolun her adımını görün.", en: "Resolve nested JSON values with RFC 6901 pointers and inspect each path step." }, description: { tr: "Bir JSON belgesinde /users/0/name gibi RFC 6901 uyumlu yolları çözer; ~0 ve ~1 kaçışlarını işler, ulaşılamayan segmenti açıkça bildirir ve bulunan değerin türünü gösterir.", en: "Resolves RFC 6901 paths such as /users/0/name in a JSON document, handles ~0 and ~1 escaping, identifies the first unreachable segment, and reports the resolved value type." }, useCases: { tr: ["API yanıtında alan bulma", "JSON Patch yolu hazırlama", "İç içe yapı hata ayıklama"], en: ["Finding fields in API responses", "Preparing JSON Patch paths", "Debugging nested structures"] }, steps: { tr: ["JSON belgesini ve Pointer yolunu girin.", "Yolu çözümleyip segment izini inceleyin.", "Bulunan değeri doğrulayıp kopyalayın."], en: ["Enter the JSON document and Pointer path.", "Resolve it and inspect the segment trace.", "Verify and copy the located value."] } },
  { slug: "sql-bicimlendirici-analizoru", category: "data", mark: "92", title: { tr: "SQL Biçimlendirici ve Sorgu Analizörü", en: "SQL Formatter & Query Analyzer" }, short: { tr: "SQL sorgularını okunur hale getirin ve riskli yazım örüntülerini yerelde ön tarayın.", en: "Format SQL queries and locally pre-screen risky query patterns." }, description: { tr: "SELECT, JOIN, WHERE ve benzeri anahtar kelimeleri düzenleyerek SQL'i okunur hale getirir; UPDATE/DELETE için WHERE eksikliği, SELECT * ve birden fazla ifade gibi gözden geçirilmesi gereken noktaları açıklar. Sorguyu çalıştırmaz ve veritabanına bağlanmaz.", en: "Formats common SQL clauses and flags review points such as UPDATE or DELETE without WHERE, SELECT *, and multiple statements. It never executes the query or connects to a database." }, useCases: { tr: ["Kod inceleme öncesi sorgu düzenleme", "Riskli DELETE/UPDATE kontrolü", "SQL eğitim örnekleri"], en: ["Formatting queries before review", "Checking risky DELETE/UPDATE statements", "SQL learning examples"] }, steps: { tr: ["Yetkili olduğunuz sorguyu yapıştırın.", "Biçimlendirme ve kural tabanlı analizi çalıştırın.", "Uyarıları veritabanı şemanız ve sorgu planıyla doğrulayın."], en: ["Paste a query you are authorized to review.", "Run formatting and rule-based analysis.", "Verify warnings against your schema and query plan."] } },
  { slug: "openapi-endpoint-envanteri", category: "data", mark: "93", title: { tr: "OpenAPI Endpoint Envanteri", en: "OpenAPI Endpoint Inventory" }, short: { tr: "OpenAPI JSON belgelerinden yöntem, yol, etiket ve güvenlik özetini çıkarın.", en: "Extract method, path, tag, and security summaries from OpenAPI JSON." }, description: { tr: "OpenAPI 3.x JSON belgesinin paths bölümünü tarar; endpoint yöntemlerini, operationId, etiket, güvenlik ve deprecated durumunu tabloya dönüştürür. Ağ isteği yapmadan API kapsam incelemesini hızlandırır.", en: "Scans the paths object of an OpenAPI 3.x JSON document and turns methods, operationId, tags, security, and deprecated status into an inventory without making network requests." }, useCases: { tr: ["API kapsam envanteri", "Deprecated endpoint bulma", "Güvenlik tanımı ön kontrolü"], en: ["API scope inventory", "Finding deprecated endpoints", "Pre-checking security declarations"] }, steps: { tr: ["OpenAPI JSON belgesini yükleyin.", "Endpoint envanterini oluşturun.", "Eksik güvenlik ve operationId alanlarını kaynak kodda doğrulayın."], en: ["Load an OpenAPI JSON document.", "Generate the endpoint inventory.", "Verify missing security and operationId fields in source code."] } },
  { slug: "env-dosyasi-denetleyici", category: "codeSecurity", mark: "94", title: { tr: ".env Dosyası Güvenlik Denetleyici", en: ".env Security Auditor" }, short: { tr: "Ortam değişkenlerini değerleri açığa çıkarmadan biçim, tekrar ve sır riski açısından inceleyin.", en: "Inspect environment variables for format, duplicates, and secret risk without revealing values." }, description: { tr: ".env içeriğini tarayıcıda ayrıştırır; yinelenen anahtar, zayıf adlandırma, boş kritik değer, canlı sır paylaşımı ve istemciye açılabilecek önekleri raporlar. Değerleri çıktıda maskeler; sızıntı tespiti veya gerçek sır doğrulaması garantisi vermez.", en: "Parses .env content in-browser and reports duplicate keys, weak names, empty critical values, likely live secrets, and client-exposed prefixes. Output values are masked; this is not a leak-detection or credential-validation guarantee." }, useCases: { tr: ["Commit öncesi .env kontrolü", "Örnek ortam dosyası hazırlama", "İstemci öneklerini denetleme"], en: ["Pre-commit .env review", "Preparing example environment files", "Auditing client-exposed prefixes"] }, steps: { tr: ["Gerçek sırları mümkünse sahte değerlerle değiştirip içeriği yapıştırın.", "Yerel denetimi çalıştırın ve maskeli bulguları inceleyin.", "Gerçek sızıntı şüphesinde anahtarı döndürün ve geçmişi ayrıca tarayın."], en: ["Prefer replacing real secrets with synthetic values before pasting.", "Run the local audit and inspect masked findings.", "Rotate credentials and audit history separately if exposure is suspected."] } },
  { slug: "unix-izin-hesaplayici", category: "codeSecurity", mark: "95", title: { tr: "Unix İzin Hesaplayıcı", en: "Unix Permission Calculator" }, short: { tr: "chmod sayısal ve rwx gösterimlerini iki yönde dönüştürüp riskli bitleri açıklayın.", en: "Convert chmod octal and rwx notation both ways and explain risky bits." }, description: { tr: "755, 0644 veya rwxr-xr-x gibi Unix dosya izinlerini dönüştürür; sahip, grup ve diğer yetkilerini açıklar, world-writable ve setuid/setgid gibi riskli durumları işaretler. Sunucuda izin değiştirmez.", en: "Converts Unix permissions such as 755, 0644, or rwxr-xr-x, explains owner/group/other access, and flags world-writable or special-bit risk. It never changes server permissions." }, useCases: { tr: ["chmod komutu hazırlama", "Dağıtım dosyası izin kontrolü", "Linux izinlerini öğrenme"], en: ["Preparing chmod commands", "Reviewing deployment file permissions", "Learning Linux permission notation"] }, steps: { tr: ["Sayısal veya rwx gösterimini girin.", "Dönüşümü ve yetki matrisini inceleyin.", "Değişikliği uygulamadan önce dosyanın rolünü ve tehdit modelini doğrulayın."], en: ["Enter octal or rwx notation.", "Review the conversion and access matrix.", "Verify file purpose and threat model before applying changes."] } },
  { slug: "renk-kontrast-denetleyici", category: "converter", mark: "96", title: { tr: "Renk Kontrastı Denetleyici", en: "Color Contrast Checker" }, short: { tr: "İki rengi WCAG kontrast oranı ve metin eşikleriyle karşılaştırın.", en: "Compare two colors against WCAG contrast ratios and text thresholds." }, description: { tr: "Ön plan ve arka plan HEX renkleri için göreli parlaklık ile kontrast oranını hesaplar; normal ve büyük metin için AA/AAA eşiklerini ayrı gösterir. Sonuç, gerçek arayüzü ve kullanıcı testini tek başına yerine geçirmez.", en: "Calculates relative luminance and contrast ratio for foreground and background HEX colors, then reports AA and AAA thresholds for normal and large text separately. It does not replace testing the real interface." }, useCases: { tr: ["Tasarım sistemi renk kontrolü", "Karanlık tema okunabilirliği", "CTA metni erişilebilirliği"], en: ["Design-system color checks", "Dark-mode readability", "CTA text accessibility"] }, steps: { tr: ["Ön plan ve arka plan HEX renklerini girin.", "Kontrast oranını ve eşikleri hesaplayın.", "Gerçek yazı boyutu, ağırlığı ve durum renkleriyle ayrıca test edin."], en: ["Enter foreground and background HEX colors.", "Calculate the ratio and thresholds.", "Test actual font size, weight, and state colors as well."] } },
  { slug: "en-boy-orani-hesaplayici", category: "calculation", mark: "97", title: { tr: "En-Boy Oranı Hesaplayıcı", en: "Aspect Ratio Calculator" }, short: { tr: "Görsel/video oranını sadeleştirin ve yeni genişlik ya da yüksekliği doğru ölçekleyin.", en: "Reduce image/video ratios and scale a new width or height accurately." }, description: { tr: "Piksel boyutlarından en-boy oranını en sade tam sayılara indirger ve hedef genişlik ya da yüksekliğe göre diğer boyutu hesaplar. Yuvarlama farkını açıkça gösterir.", en: "Reduces pixel dimensions to the simplest integer aspect ratio and calculates the missing dimension from a target width or height, with explicit rounding." }, useCases: { tr: ["Sosyal görsel boyutlandırma", "Video kapak oranı", "Responsive yer tutucu hesabı"], en: ["Sizing social images", "Video cover ratios", "Responsive placeholder calculations"] }, steps: { tr: ["Kaynak genişlik ve yüksekliği girin.", "İsteğe bağlı hedef boyutu ekleyin.", "Oran ve ölçeklenmiş sonucu doğrulayın."], en: ["Enter source width and height.", "Optionally add a target dimension.", "Verify the ratio and scaled result."] } },
  { slug: "css-clamp-olusturucu", category: "data", mark: "98", title: { tr: "CSS clamp() Oluşturucu", en: "CSS clamp() Generator" }, short: { tr: "Akışkan tipografi ve boşluklar için sınırları açıklanan clamp() ifadesi üretin.", en: "Generate a bounded clamp() expression for fluid type and spacing." }, description: { tr: "Minimum/maksimum değer ile viewport aralığından doğrusal bir CSS clamp() ifadesi üretir; eğim, kesişim ve uç noktaları gösterir. Kullanılan birimlerin ve tasarım ölçeğinin uyumunu sizin doğrulamanız gerekir.", en: "Builds a linear CSS clamp() expression from minimum/maximum values and viewport bounds, showing slope, intercept, and endpoints. You remain responsible for validating units and design scale." }, useCases: { tr: ["Akışkan yazı boyutu", "Responsive boşluk", "Tasarım tokenı üretme"], en: ["Fluid typography", "Responsive spacing", "Generating design tokens"] }, steps: { tr: ["Değer ve viewport sınırlarını JSON örneğinde düzenleyin.", "clamp() ifadesini üretin.", "Ara ve uç ekran genişliklerinde görsel test yapın."], en: ["Edit value and viewport bounds in the JSON example.", "Generate the clamp() expression.", "Visually test intermediate and endpoint widths."] } },
  { slug: "is-gunu-hesaplayici", category: "calculation", mark: "99", title: { tr: "İş Günü Hesaplayıcı", en: "Business Day Calculator" }, short: { tr: "İki tarih arasındaki hafta içi günleri isteğe bağlı tatil listesiyle hesaplayın.", en: "Count weekdays between dates with an optional holiday list." }, description: { tr: "Başlangıç ve bitiş tarihleri arasındaki pazartesi-cuma günlerini, yerel saat dilimi kaymalarına karşı UTC tabanlı hesaplar; isteğe bağlı ISO tatil tarihlerini hariç tutar. Resmî çalışma takvimi veya hukuki süre hesabı değildir.", en: "Counts Monday-to-Friday dates between two endpoints using UTC-safe date arithmetic and excludes optional ISO holiday dates. It is not an official employment calendar or legal deadline calculation." }, useCases: { tr: ["Proje süresi tahmini", "Teslim günü karşılaştırma", "SLA ön hesabı"], en: ["Project-duration estimates", "Delivery-date comparison", "Preliminary SLA calculations"] }, steps: { tr: ["Başlangıç, bitiş ve isteğe bağlı tatilleri girin.", "Dahil etme kuralıyla hesabı çalıştırın.", "Resmî tatilleri ve hukuki kuralları yetkili kaynaktan doğrulayın."], en: ["Enter start, end, and optional holidays.", "Run the calculation with the stated inclusion rule.", "Verify official holidays and legal rules with an authoritative source."] } },
  { slug: "zaman-dilimi-toplanti-planlayici", category: "general", mark: "100", title: { tr: "Zaman Dilimi Toplantı Planlayıcı", en: "Time Zone Meeting Planner" }, short: { tr: "Tek bir saati birden fazla IANA zaman diliminde karşılaştırmalı görün.", en: "Compare one meeting time across multiple IANA time zones." }, description: { tr: "Bir ISO tarih-saatini Europe/Istanbul, America/New_York gibi IANA bölgelerinde gösterir ve her katılımcı için yerel gün/saat görünümü üretir. Tarayıcının saat dilimi verisini kullanır; davet veya bildirim göndermez.", en: "Displays one ISO date-time across IANA zones such as Europe/Istanbul and America/New_York, producing a local day/time view for each participant. It uses browser time-zone data and sends no invites." }, useCases: { tr: ["Dağıtık ekip toplantısı", "Webinar saat duyurusu", "Yaz saati farkı kontrolü"], en: ["Distributed-team meetings", "Webinar time announcements", "Daylight-saving checks"] }, steps: { tr: ["ISO tarih-saat ve IANA bölgelerini girin.", "Yerel saat tablosunu oluşturun.", "Davetten önce katılımcılarla tarih ve saat dilimini teyit edin."], en: ["Enter an ISO date-time and IANA zones.", "Generate the local-time table.", "Confirm the date and zone with participants before inviting."] } },
  { slug: "bahsis-hesap-bolusturucu", category: "calculation", mark: "101", title: { tr: "Bahşiş ve Hesap Bölüştürücü", en: "Tip & Bill Splitter" }, short: { tr: "Vergi, bahşiş ve kişi sayısını şeffaf formülle hesaplayıp eşit bölüştürün.", en: "Calculate tax, tip, and per-person totals with a transparent formula." }, description: { tr: "Ara toplam, vergi, bahşiş yüzdesi ve kişi sayısından toplamı ve kişi başı tutarı hesaplar; bahşiş tabanının vergi öncesi mi sonrası mı olduğunu açıkça belirtir. Para birimi dönüşümü veya ödeme yapmaz.", en: "Calculates total and per-person amount from subtotal, tax, tip rate, and party size, explicitly stating whether tip uses the pre- or post-tax base. It performs no currency conversion or payment." }, useCases: { tr: ["Restoran hesabı paylaşma", "Etkinlik gideri bölme", "Bahşiş senaryosu karşılaştırma"], en: ["Splitting restaurant bills", "Sharing event expenses", "Comparing tip scenarios"] }, steps: { tr: ["Tutarları, oranı ve kişi sayısını girin.", "Hesaplama tabanını seçip sonucu üretin.", "Yuvarlama ve yerel uygulamayı ödeme öncesi kontrol edin."], en: ["Enter amounts, rate, and party size.", "Choose the tip base and calculate.", "Check rounding and local practice before payment."] } },
  { slug: "regex-degistirme-laboratuvari", category: "text", mark: "102", title: { tr: "Regex Değiştirme Laboratuvarı", en: "Regex Replacement Lab" }, short: { tr: "Yakalama gruplarıyla metin dönüşümünü önizleyin ve değişen eşleşmeleri sayın.", en: "Preview capture-group replacements and count changed matches." }, description: { tr: "Desen, bayraklar ve değiştirme şablonunu ayrı alanlarda çalıştırır; $1 gibi yakalama gruplarını önizler, eşleşme sayısını ve önce/sonra çıktısını gösterir. Felaket geri izlemeyi sınırlamak için giriş boyutu sınırlandırılmıştır.", en: "Runs pattern, flags, and replacement separately, previews capture groups such as $1, and reports matches with before/after output. Input is bounded to reduce catastrophic-backtracking risk." }, useCases: { tr: ["Toplu adlandırma taslağı", "Log satırı yeniden biçimlendirme", "Yakalama grubu öğrenme"], en: ["Drafting bulk renames", "Reformatting log lines", "Learning capture groups"] }, steps: { tr: ["Metin, desen, bayrak ve değiştirmeyi girin.", "Sınırlı önizlemeyi çalıştırın.", "Beklenmeyen eşleşmeleri önce/sonra görünümünde kontrol edin."], en: ["Enter text, pattern, flags, and replacement.", "Run the bounded preview.", "Check unexpected matches in the before/after view."] } },
  { slug: "markdown-tablo-olusturucu", category: "general", mark: "103", title: { tr: "Markdown Tablo Oluşturucu", en: "Markdown Table Generator" }, short: { tr: "JSON dizisi veya CSV verisini hizalı, kaçışları doğru Markdown tablosuna çevirin.", en: "Turn a JSON array or CSV data into an escaped, aligned Markdown table." }, description: { tr: "Nesne dizisi biçimindeki JSON veya basit CSV girdisinden Markdown tablo üretir; dikey çizgi ve satır sonlarını kaçırır, eksik alanları tutarlı boş hücrelerle tamamlar. Karmaşık CSV tırnaklama kuralları için önce CSV İnceleyici kullanılmalıdır.", en: "Generates a Markdown table from a JSON object array or simple CSV, escaping pipes and line breaks and filling missing fields consistently. Use CSV Inspector first for complex CSV quoting rules." }, useCases: { tr: ["README tablosu hazırlama", "Rapor verisini Markdown'a çevirme", "Dokümantasyon karşılaştırma tablosu"], en: ["Preparing README tables", "Converting report data to Markdown", "Documentation comparison tables"] }, steps: { tr: ["JSON dizisi veya basit CSV yapıştırın.", "Tabloyu oluşturup başlık ve kaçışları inceleyin.", "Önizleme hedefinizde son görünümü doğrulayın."], en: ["Paste a JSON array or simple CSV.", "Generate the table and inspect headers and escaping.", "Verify final rendering in the target preview."] } },
...
export const tools: Tool[] = [...baseTools, ...demandTools, ...discoveryTools, ...productivityTools, ...expansionTools, ...essentialTools, ...precisionTools, ...frontierTools, ...stageTwoTools, ...studioTools, ...insightTools].map(localizeTool);
...
const relatedBySlug: Record<string, string[]> = {
  ...
  "yaml-json-donusturucu": ["json-bicimlendirici", "json-schema-olusturucu", "xml-bicimlendirici-dogrulayici"],
  "xml-bicimlendirici-dogrulayici": ["yaml-json-donusturucu", "json-bicimlendirici", "html-varlik-kodlayici"],
  "json-flatten-unflatten": ["json-bicimlendirici", "json-diff-karsilastirma", "csv-tekil-satir-ayiklayici"],
  "csv-tekil-satir-ayiklayici": ["csv-inceleyici", "json-csv-donusturucu", "json-flatten-unflatten"],
  "url-sorgu-parametresi-analizoru": ["utm-kampanya-url-olusturucu", "url-kodlayici", "url-guvenlik-on-kontrolu"],
  "html-varlik-kodlayici": ["markdown-onizleyici", "xml-bicimlendirici-dogrulayici", "url-kodlayici"],
  "ip-cidr-alt-ag-hesaplayici": ["url-guvenlik-on-kontrolu", "csp-olusturucu-denetleyici", "birim-donusturucu"],
  "robots-txt-olusturucu-denetleyici": ["hreflang-etiket-olusturucu", "meta-etiket-favicon-uretici", "csp-olusturucu-denetleyici"],
  "hreflang-etiket-olusturucu": ["robots-txt-olusturucu-denetleyici", "faq-json-ld-olusturucu", "meta-etiket-favicon-uretici"],
  "faq-json-ld-olusturucu": ["hreflang-etiket-olusturucu", "json-bicimlendirici", "meta-etiket-favicon-uretici"],
  "utm-kampanya-url-olusturucu": ["url-sorgu-parametresi-analizoru", "qr-kod-olusturucu", "url-kodlayici"],
  "unicode-normalizasyon-inceleyici": ["seo-slug-olusturucu", "metin-temizleyici", "kelime-sikligi-ngram-analizi"],
  "satir-siralayici-tekillestirici": ["csv-tekil-satir-ayiklayici", "metin-temizleyici", "kelime-sikligi-ngram-analizi"],
  "seo-slug-olusturucu": ["unicode-normalizasyon-inceleyici", "url-kodlayici", "hreflang-etiket-olusturucu"],
  "kelime-sikligi-ngram-analizi": ["kelime-sayaci", "okunabilirlik-analizi", "yerel-metin-ozetleyici"],
  "yuzde-degisim-hesaplayici": ["kdv-indirim-hesaplayici", "yatirim-getiri-simulatoru", "birim-donusturucu"],
  "kdv-indirim-hesaplayici": ["yuzde-degisim-hesaplayici", "fatura-sablonu-olusturucu", "yatirim-getiri-simulatoru"],
  "sure-mesai-hesaplayici": ["tarih-farki-hesaplayici", "yuzde-degisim-hesaplayici", "birim-donusturucu"],
  "rastgele-secici-takim-karistirici": ["uuid-uretici", "hatirlanabilir-parola-uretici", "satir-siralayici-tekillestirici"],
  "hatirlanabilir-parola-uretici": ["guclu-parola-uretici", "sifre-gucu-testi", "hmac-olusturucu-dogrulayici"],
  "hmac-olusturucu-dogrulayici": ["sri-butunluk-hash-uretici", "sha256-ozet-uretici", "dosya-hash-karsilastirici"],
  "sri-butunluk-hash-uretici": ["hmac-olusturucu-dogrulayici", "dosya-hash-karsilastirici", "csp-olusturucu-denetleyici"],
  "rag-parcalama-butcesi-planlayici": ["token-baglam-butcesi-planlayici", "token-sayaci", "prompt-enjeksiyon-on-taramasi"],
  "prompt-enjeksiyon-on-taramasi": ["rag-parcalama-butcesi-planlayici", "sistem-promptu-netlik-kontrolu", "kod-guvenligi-on-taramasi"],
  "prompt-test-vaka-matrisi": ["prompt-kalite-denetimi", "ai-yanit-degerlendirme-rubrigi", "prompt-enjeksiyon-on-taramasi"],
  "data-uri-donusturucu": ["base64-kodlayici", "html-varlik-kodlayici", "dosya-risk-on-taramasi"],
  "http-guvenlik-basliklari-denetleyici": ["csp-olusturucu-denetleyici", "robots-txt-olusturucu-denetleyici", "url-guvenlik-on-kontrolu"],
};
