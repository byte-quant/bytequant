...
const translations: Record<string, TranslationPair> = {
  ...
  "metin-temizleyici": {
    ...
    zh: { title: "文本清理器", short: "清除多余空格、重复空行和不一致格式。", description: "在当前浏览器中清理复制文本的多余空格、重复空行与换行问题，并提供处理前后对比，便于确认原意、段落边界和重要字符未被意外改变。" },
  },
  ...
  "json-lines-donusturucu": { de: { title: "JSON-Lines-Konverter", short: "JSON-Arrays und zeilenweises NDJSON sicher umwandeln.", description: "Validiert NDJSON Datensatz für Datensatz und wandelt JSON-Arrays in JSON Lines oder zurück um. Fehler werden mit Zeilennummer gemeldet; Daten werden nicht hochgeladen." }, zh: { title: "JSON Lines 转换器", short: "在 JSON 数组与逐行 NDJSON 之间安全转换。", description: "逐行验证 NDJSON，并在 JSON 数组与 JSON Lines 之间转换；错误会标明行号，数据不会上传。" } },
  "json-pointer-gezgini": { de: { title: "JSON-Pointer-Explorer", short: "Verschachtelte Werte mit RFC-6901-Pfaden auflösen.", description: "Löst RFC-6901-Pfade in JSON auf, verarbeitet ~0/~1-Escapes und zeigt das erste unerreichbare Segment sowie den Werttyp." }, zh: { title: "JSON Pointer 导航器", short: "使用 RFC 6901 路径定位嵌套 JSON 值。", description: "解析 JSON 中的 RFC 6901 路径，支持 ~0/~1 转义，并显示首个不可达片段与结果类型。" } },
  "sql-bicimlendirici-analizoru": { de: { title: "SQL-Formatierer & Abfrageanalyse", short: "SQL lesbar formatieren und riskante Muster lokal vorprüfen.", description: "Formatiert häufige SQL-Klauseln und markiert SELECT *, mehrere Anweisungen sowie UPDATE/DELETE ohne WHERE. Keine Abfrage wird ausgeführt." }, zh: { title: "SQL 格式化与查询分析器", short: "格式化 SQL 并在本地预检查风险模式。", description: "格式化常见 SQL 子句，并提示 SELECT *、多语句及缺少 WHERE 的 UPDATE/DELETE；不会执行查询。" } },
  "openapi-endpoint-envanteri": { de: { title: "OpenAPI-Endpunktinventar", short: "Methoden, Pfade, Tags und Sicherheit aus OpenAPI-JSON extrahieren.", description: "Erstellt aus OpenAPI-3.x-JSON ein lokales Inventar von Methoden, Pfaden, operationId, Tags, Sicherheit und veralteten Endpunkten." }, zh: { title: "OpenAPI 端点清单", short: "从 OpenAPI JSON 提取方法、路径、标签与安全摘要。", description: "从 OpenAPI 3.x JSON 本地生成方法、路径、operationId、标签、安全声明与弃用状态清单。" } },
  "env-dosyasi-denetleyici": { de: { title: ".env-Sicherheitsprüfer", short: "Format, Duplikate und Geheimnisrisiken prüfen, ohne Werte offenzulegen.", description: "Meldet doppelte Schlüssel, schwache Namen, leere kritische Werte und Client-Präfixe; Ausgabewerte werden maskiert. Keine Leakerkennungsgarantie." }, zh: { title: ".env 安全审计器", short: "在不暴露值的情况下检查格式、重复与密钥风险。", description: "报告重复键、弱命名、空关键值与客户端前缀，并遮蔽输出值；不构成泄漏检测保证。" } },
  "unix-izin-hesaplayici": { de: { title: "Unix-Berechtigungsrechner", short: "chmod-Zahlen und rwx-Notation umwandeln und Risiken erklären.", description: "Wandelt 755, 0644 oder rwxr-xr-x um, erklärt Besitzer/Gruppe/Andere und markiert riskante Schreib- oder Spezialbits. Es ändert keine Dateien." }, zh: { title: "Unix 权限计算器", short: "转换 chmod 八进制与 rwx 表示，并解释风险位。", description: "转换 755、0644 或 rwxr-xr-x，解释所有者/组/其他权限并提示危险写入或特殊位；不会修改文件。" } },
  "renk-kontrast-denetleyici": { de: { title: "Farbkontrast-Prüfer", short: "Zwei Farben anhand von WCAG-Kontrast und Textschwellen vergleichen.", description: "Berechnet relative Leuchtdichte und Kontrastverhältnis und zeigt AA/AAA für normalen und großen Text getrennt. Reale UI-Tests bleiben erforderlich." }, zh: { title: "颜色对比度检查器", short: "按 WCAG 对比度与文本阈值比较两种颜色。", description: "计算相对亮度与对比度，分别显示普通和大号文本的 AA/AAA 结果；仍需测试真实界面。" } },
  "en-boy-orani-hesaplayici": { de: { title: "Seitenverhältnis-Rechner", short: "Bild-/Videoverhältnis kürzen und neue Abmessungen skalieren.", description: "Kürzt Pixelmaße auf das kleinste ganzzahlige Verhältnis und berechnet aus Zielbreite oder -höhe die fehlende Dimension mit transparenter Rundung." }, zh: { title: "宽高比计算器", short: "简化图像/视频比例并正确缩放新尺寸。", description: "把像素尺寸化简为最小整数比例，并根据目标宽度或高度计算另一边，同时说明取整。" } },
  "css-clamp-olusturucu": { de: { title: "CSS-clamp()-Generator", short: "Begrenztes clamp() für fließende Schrift und Abstände erzeugen.", description: "Erzeugt aus Min-/Max-Werten und Viewport-Grenzen ein lineares CSS clamp() und zeigt Steigung, Achsenabschnitt sowie Endpunkte." }, zh: { title: "CSS clamp() 生成器", short: "为流式字号与间距生成有边界的 clamp()。", description: "根据最小/最大值和视口边界生成线性 CSS clamp()，并显示斜率、截距与端点。" } },
  "is-gunu-hesaplayici": { de: { title: "Arbeitstage-Rechner", short: "Wochentage zwischen Daten mit optionalen Feiertagen zählen.", description: "Zählt Montag bis Freitag UTC-sicher und schließt optionale ISO-Feiertage aus. Kein offizieller Arbeitskalender oder Rechtsfristenrechner." }, zh: { title: "工作日计算器", short: "计算日期之间的周一至周五，可排除节假日。", description: "使用 UTC 安全日期运算计算工作日并排除可选 ISO 节假日；不属于官方工作日历或法律期限计算。" } },
  "zaman-dilimi-toplanti-planlayici": { de: { title: "Zeitzonen-Terminplaner", short: "Eine Uhrzeit in mehreren IANA-Zeitzonen vergleichen.", description: "Zeigt einen ISO-Termin in mehreren IANA-Zonen mit lokalem Tag und Uhrzeit. Nutzt Browser-Zeitzonendaten und versendet keine Einladungen." }, zh: { title: "时区会议规划器", short: "在多个 IANA 时区中比较同一会议时间。", description: "把一个 ISO 日期时间显示为多个 IANA 时区的本地日期与时间；使用浏览器数据，不发送邀请。" } },
  "bahsis-hesap-bolusturucu": { de: { title: "Trinkgeld- & Rechnungsaufteiler", short: "Steuer, Trinkgeld und Betrag pro Person transparent berechnen.", description: "Berechnet aus Zwischensumme, Steuer, Trinkgeldsatz und Personenzahl Gesamt- und Pro-Kopf-Betrag und nennt die Trinkgeldbasis." }, zh: { title: "小费与账单分摊器", short: "用透明公式计算税费、小费和人均金额。", description: "根据小计、税费、小费比例和人数计算总额及人均金额，明确小费按税前还是税后金额计算，并显示舍入差异；该工具不会换汇或代为付款。" } },
  "regex-degistirme-laboratuvari": { de: { title: "Regex-Ersetzungslabor", short: "Ersetzungen mit Gruppen voranzeigen und Treffer zählen.", description: "Führt Muster, Flags und Ersetzung getrennt aus, zeigt Gruppen wie $1 sowie Treffer und Vorher/Nachher. Eingaben sind zur Risikobegrenzung beschränkt." }, zh: { title: "正则替换实验室", short: "预览捕获组替换并统计变更匹配。", description: "分别运行模式、标志和替换，预览 $1 等捕获组并显示匹配及前后结果；输入有大小限制。" } },
  "markdown-tablo-olusturucu": { de: { title: "Markdown-Tabellengenerator", short: "JSON-Arrays oder CSV in korrekt escapte Markdown-Tabellen umwandeln.", description: "Erzeugt aus Objekt-JSON oder einfachem CSV eine Markdown-Tabelle, maskiert Pipes und Zeilenumbrüche und ergänzt fehlende Zellen." }, zh: { title: "Markdown 表格生成器", short: "把 JSON 数组或 CSV 转换为正确转义的 Markdown 表格。", description: "从对象 JSON 或简单 CSV 生成 Markdown 表格，转义竖线与换行并一致补齐缺失单元格。" } },
  "semver-karsilastirici": { de: { title: "SemVer-Vergleicher", short: "Versionen nach SemVer- und Vorabversionsregeln vergleichen und sortieren.", description: "Validiert und sortiert major.minor.patch samt Vorabkennungen nach SemVer. Build-Metadaten ändern die Priorität nicht; Kompatibilität wird nicht bewiesen." }, zh: { title: "SemVer 比较器", short: "按 SemVer 预发布规则比较并排序版本。", description: "按 SemVer 验证和排序 major.minor.patch 及预发布标识；构建元数据不影响优先级，也不能单独证明兼容性。" } },
};
...
export function localizeTool(base: BaseTool): Tool {
  ...
  // Preserve authored instructions, including inline German/Chinese copy.
  // Appending the title and a category paragraph to every step hid useful copy.
  const useCases = { tr: base.useCases.tr, en: base.useCases.en, de: base.useCases.de ?? guidance.useCases.de, zh: base.useCases.zh ?? guidance.useCases.zh };
  const steps = { tr: base.steps.tr, en: base.steps.en, de: base.steps.de ?? guidance.steps.de, zh: base.steps.zh ?? guidance.steps.zh };
