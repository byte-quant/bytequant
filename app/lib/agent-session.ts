...
export const AGENT_VERSION = "BQ-Agent 1.0";
...
export function readAgentSession(raw: string | null): AgentSession | null {
  ...
  try {
    ...
    if (plan?.version !== AGENT_VERSION || !(["tr", "en", "de", "zh"] as string[]).includes(plan.locale) || typeof plan.goal !== "string" || plan.goal.length > 20_000 || !Array.isArray(plan.steps) || plan.steps.length < 1 || plan.steps.length > 6 || !Number.isInteger(value.currentStep)) return null;
    ...
    if (!validSteps || typeof value.stepOutputs !== "object" || value.stepOutputs === null || Array.isArray(value.stepOutputs)) return null;
