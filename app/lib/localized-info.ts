  ...
  zh: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { heading: "4. 本地存储、项目动态与同意", paragraphs: ["bq-consent-v1 保存 180 天的选择，bq-theme 保存所选主题。bq-tool-usage-v1 与 bq-tool-favorites-v1 仅在同意后启用，只包含工具标识、次数、最后使用时间或收藏，不包含内容。", "主页的项目健康卡是静态内容，不会调用 GitHub API。社区草稿在本标签页预检查，并可通过受限链接、Markdown 文件或设备分享功能免账号分享；GitHub 仅用于可选的长期公开发布。", "当前没有启用分析、广告或跟踪 SDK。如未来启用，会先更新说明和有效同意流程。"] },
    ...
    faq: {
      ...
      sections: [
        ...
        { heading: "德语和中文是否完整支持？", paragraphs: ["导航、全部 104 个工具页面、核心页面、元数据和结构化数据已本地化。扩展后的工作流指南库提供德语与中文版本；其他既有文章会明确标注为英文原文。"] },
