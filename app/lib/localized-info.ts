  ...
  de: {
    ...
    terms: {
      ...
      sections: [
        ...
        { heading: "Verfügbarkeit und Haftung", paragraphs: ["Der Dienst wird ohne Verfügbarkeits- oder Fehlerfreiheitsgarantie bereitgestellt. Prüfen und sichern Sie Ergebnisse vor wichtiger Nutzung. Soweit gesetzlich zulässig, liegt die Verantwortung für Eingabe, Interpretation und Verwendung bei Ihnen."] },
      ],
    },
    contact: {
      eyebrow: "KONTAKT",
    ...
    faq: {
      ...
      sections: [
        ...
        { heading: "Wie installiere ich ByteQuant?", paragraphs: ["Nutzen Sie die Schaltfläche „App installieren“ oder den Browserbefehl „Zum Startbildschirm hinzufügen“. Auf iOS erfolgt dies in Safari über Teilen → Zum Home-Bildschirm."] },
        ...
        { heading: "Sind Deutsch und Chinesisch vollständig unterstützt?", paragraphs: ["Navigation, Werkzeugkatalog, Werkzeugbeschreibungen, Kernseiten, Metadaten und strukturierte Daten sind lokalisiert. Bestehende Langform-Ratgeber bleiben vorerst Türkisch/Englisch und werden als solche verlinkt."] },
  ...
  zh: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { heading: "3. PWA、Service Worker 与离线缓存", paragraphs: ["可安装 Web 应用只缓存同源应用资源和已访问的 GET 页面。工具输入、生成结果、密码和所选文件绝不会写入离线缓存。", "您可在浏览器或应用设置中移除 Service Worker。安装图标不会改变本地隐私模型。"] },
    ...
    terms: {
      ...
      sections: [
        ...
        { heading: "可用性与责任", paragraphs: ["服务不保证持续可用或没有错误。重要使用前请核验并备份结果。在法律允许范围内，输入、解释和使用责任由用户承担。"] },
      ],
    },
    contact: {
      eyebrow: "联系",
    ...
    faq: {
      ...
      sections: [
        ...
        { heading: "如何安装 ByteQuant？", paragraphs: ["使用“安装应用”按钮，或浏览器的“添加到主屏幕”。在 iOS Safari 中选择“分享”→“添加到主屏幕”。"] },
        { heading: "离线时哪些功能可用？", paragraphs: ["应用外壳和已访问且已缓存的页面可离线打开。未访问页面需要联网。工具输入绝不会被离线保存。"] },
        { heading: "德语和中文是否完整支持？", paragraphs: ["导航、工具目录、工具说明、核心页面、元数据和结构化数据已本地化。现有长篇指南暂时保留土耳其语/英语，并会明确标注链接。"] },
