  ...
  zh: {
    ...
    cookies: {
      ...
      sections: [
        ...
        { heading: "本地助手会话与可选模型缓存", paragraphs: ["bytequant:local-agent:v1 与 bytequant:local-agent-conversation:v5 只存在于当前标签页的 sessionStorage，可包含目标、计划及最多十二条对话摘要，并随标签页会话结束。", "WebLLM 缓存只在您启用本地 AI 后创建，包含模型权重、分词器和 WebAssembly，不包含对话内容；可能占用数百 MB，可通过清除本站浏览器数据移除。"] },
        { heading: "工作站项目与临时传递", paragraphs: ["bytequant-workspaces 在 IndexedDB 中保存您明确存储的 AES-GCM-256 加密项目和不可导出的设备密钥，直至您删除项目或站点数据。bytequant:workstation-active:v1 与 bytequant:workstation-handoff:v1 是标签页级 sessionStorage 记录；浏览器配额和设备丢失仍是本地风险。"] },
      ],
    },
    terms: {
      eyebrow: "使用条款",
