  ...
  de: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { heading: "4. Lokaler Speicher, globale Community, Meldungen und Einwilligung", paragraphs: ["bq-consent-v1 speichert die Auswahl für 180 Tage und bq-theme das Farbschema. bq-tool-usage-v1 sowie bq-tool-favorites-v1 werden nur nach Einwilligung aktiviert und enthalten Werkzeugkennungen, Zähler, Zeitpunkte oder Favoriten – nie Eingaben oder Ergebnisse. bytequant:news-favorites:v1 enthält nur ausdrücklich gespeicherte Karten-IDs.", "Die globale Community verbindet sich erst nach Ihrer ausdrücklichen Wahl über das offene Nostr-Protokoll mit ausgewählten Drittanbieter-Relays. Betreiber können IP-Adresse, Verbindungszeit und öffentliche signierte Inhalte nach eigener Richtlinie verarbeiten. ByteQuant betreibt kein Relay. Der private Profilschlüssel liegt AES-GCM-verschlüsselt im lokalen Speicher; der Schlüssel wird per PBKDF2-SHA-256 aus Ihrer Geräte-Passphrase abgeleitet. Öffentliche Beiträge können auf Relays fortbestehen und nicht überall löschbar sein. Gerätefilter und Sperrliste sind keine netzweite Moderation.", "Der Meldungsfeed übernimmt beim Build Titel, Datum, Quelle, Originallink und höchstens 24 Wörter der offiziellen Feed-Beschreibung. Fehlt sie, erscheint nur eine begrenzte, als solche erkennbare Metadaten-Notiz. Volltexte und Bilder werden nicht erneut veröffentlicht; Analytik-, Werbe- oder Tracking-SDKs sind nicht aktiv."] },
  ...
  zh: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { heading: "4. 本地存储、全球社区、动态与同意", paragraphs: ["bq-consent-v1 保存 180 天的选择，bq-theme 保存所选主题。bq-tool-usage-v1 与 bq-tool-favorites-v1 仅在同意后启用，只包含工具标识、次数、最后使用时间或收藏，不包含内容。bytequant:news-favorites:v1 仅保存您主动收藏的卡片 ID。", "只有在您主动选择连接后，全球社区才会通过开放 Nostr 协议连接所选第三方中继。运营者可按其政策处理 IP、连接时间和公开签名内容；ByteQuant 不运营中继。资料私钥以 AES-GCM 密文保存在本地，密钥由设备密码通过 PBKDF2-SHA-256 派生。公开内容可能被中继保留，无法保证从所有副本删除；设备过滤与屏蔽不是全网审核。", "动态页在构建时读取获准的官方 HTTPS Feed，展示标题、日期、来源链接及最多 24 个词的 Feed 简介；若无简介，只显示明确受限的元数据说明。ByteQuant 不转载全文或图片，且未启用分析、广告或跟踪 SDK。"] },
