  ...
  de: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { heading: "2A. Workstation, verschlüsselte Projekte, P2P und Rezeptlinks", paragraphs: ["Explizit gespeicherte Projekte bleiben AES-GCM-256-verschlüsselt in der lokalen IndexedDB-Datenbank bytequant-workspaces. Ein nicht exportierbarer Geräteschlüssel liegt in derselben Datenbank; ByteQuant kann weder Schlüssel noch Projekt remote abrufen. Dies schützt nicht vor einem kompromittierten Gerät, einer schädlichen Erweiterung oder bösartigem Code desselben Ursprungs.", "Die sessionStorage-Werte bytequant:workstation-active:v1 und bytequant:workstation-handoff:v1 können im selben Tab vorübergehend den aktiven Arbeitsbereich und begrenzten, von Ihnen gewählten Text zwischen Werkzeugen übertragen. Dateiauswahl, Passwortfelder, Codeausführung und Downloads bleiben manuell.", "Die Zusammenarbeit verwendet ausschließlich WebRTC DataChannel mit Browser-DTLS, ohne ByteQuant-Signalisierungsserver, STUN oder TURN. Einladungs- und Antwortcodes können Netzwerkinformationen enthalten; tauschen Sie sie nur über einen getrennten sicheren Kanal mit vertrauenswürdigen Personen aus. Verbundene Peers erhalten das geteilte Dokument, und NAT oder Firewalls können die Verbindung verhindern.", "Rezeptlinks schließen Ausgaben immer und Eingaben standardmäßig aus. Wenn Sie Eingaben einschließen, können diese in URL, Verlauf, Zwischenablage, Nachrichtendienst und auf dem Empfängergerät sichtbar werden."] },
    ...
    faq: {
      ...
      sections: [
        ...
        { heading: "Wie speichert und teilt die Workstation Projekte?", paragraphs: ["Gespeicherte Projekte liegen AES-GCM-verschlüsselt in IndexedDB auf diesem Gerät. WebRTC-Peers erhalten das geteilte Dokument direkt; manuelle Signalcodes können Netzwerkinformationen enthalten. Rezeptlinks schließen Eingaben standardmäßig und Ausgaben immer aus."] },
  ...
  zh: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { heading: "2A. 工作站、加密项目、P2P 与配方链接", paragraphs: ["您明确保存的项目会以 AES-GCM-256 密文保留在本机 bytequant-workspaces IndexedDB 中，不可导出的设备密钥也存于同一数据库；ByteQuant 无法远程取得密钥或项目。这不能防护已被入侵的设备、恶意扩展或同源恶意代码。", "bytequant:workstation-active:v1 与 bytequant:workstation-handoff:v1 可在同一标签页的 sessionStorage 中临时传递当前工作区及您选择的有限文本。文件选择、密码字段、代码执行和下载不会自动进行。", "协作仅使用 WebRTC DataChannel 与浏览器 DTLS，不使用 ByteQuant 信令服务器、STUN 或 TURN。邀请和应答码可能包含网络连接候选信息，只应通过独立安全渠道与可信人员交换；已连接对等方会收到共享文档，NAT 或防火墙也可能阻止连接。", "配方链接始终排除输出，默认排除输入。若主动包含输入，数据可能出现在 URL、浏览器历史、剪贴板、消息服务和接收者设备中。"] },
    ...
    faq: {
      ...
      sections: [
        ...
        { heading: "工作站如何保存和共享项目？", paragraphs: ["保存的项目会以 AES-GCM 密文留在本机 IndexedDB。WebRTC 对等方会直接收到共享文档，手动信令码可能包含网络信息；配方链接默认排除输入并始终排除输出。"] },
