...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticKeys = ["home", "tools", "agent", "workstation", "community", "news", "blog", "about", "standards", "privacy", "cookies", "terms", "contact", "faq"] as const;
  const staticRoutes = locales.flatMap((locale) => staticKeys.map((key) => ({ url: absoluteUrl(pathFor(locale, key)), lastModified: key === "home" ? new Date(`${TOOL_PAGE_UPDATED}T09:00:00+03:00`) : updated, alternates: { languages: localizedLanguageUrls(pathFor("tr", key), pathFor("en", key)) } })));
  const toolRoutes = tools.flatMap((tool) => locales.map((locale) => ({ url: absoluteUrl(toolPath(locale, tool.slug)), lastModified: new Date(`${TOOL_PAGE_UPDATED}T09:00:00+03:00`), alternates: { languages: localizedLanguageUrls(toolPath("tr", tool.slug), toolPath("en", tool.slug)) } })));
