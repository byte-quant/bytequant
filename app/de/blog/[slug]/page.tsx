...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: copy.title, description: copy.description, robots: slug === canonicalSlug ? undefined : { index: false, follow: true }, alternates: { canonical: absoluteUrl(postPath("de", canonicalSlug)), languages: { "tr-TR": absoluteUrl(postPath("tr", canonicalSlug)), "en-US": absoluteUrl(postPath("en", canonicalSlug)), "de-DE": absoluteUrl(postPath("de", canonicalSlug)), "zh-CN": absoluteUrl(postPath("zh", canonicalSlug)), "x-default": absoluteUrl(postPath("en", canonicalSlug)) } }, openGraph: { type: "article", siteName: "ByteQuant", locale: "de_DE", alternateLocale: ["tr_TR", "en_US", "zh_CN"], url: absoluteUrl(postPath("de", canonicalSlug)), title, description: copy.excerpt, publishedTime: guide.date, modifiedTime: guide.updated ?? guide.date, images: [{ url: ogImageUrl, width: 1200, height: 630, alt: title }] }, twitter: { card: "summary_large_image", title, description: copy.excerpt, images: [ogImageUrl] } };
}
