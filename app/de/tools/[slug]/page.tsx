...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: tool.title.de, description: tool.description.de, robots: isToolAlias(slug) ? nonIndexableRobots : undefined, alternates: localizedAlternates("de", toolPath("tr", canonicalSlug), toolPath("en", canonicalSlug)), openGraph: { type: "website", siteName: "ByteQuant", locale: "de_DE", alternateLocale: ["tr_TR", "en_US", "zh_CN"], url: absoluteUrl(toolPath("de", canonicalSlug)), title, description: tool.description.de, images: [{ url: ogImageUrl, width: 1200, height: 630, alt: title }] }, twitter: { card: "summary_large_image", title, description: tool.description.de, images: [ogImageUrl] } };
}
